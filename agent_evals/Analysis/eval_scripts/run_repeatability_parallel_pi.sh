...
run_single_job() {
    ...
    (
        cd "$EVALS_DIR" || exit
        pi --mode json \
            "use venv_tracelens for all commands and tool calls. Run workflow LLM eval skill on $OUTPUT_DIR for test case $id mode=$COMPARISON_SCOPE. Write results to $CASE_RESULTS/workflow_llm_results.csv"
    ...
    (
        cd "$EVALS_DIR" || exit
        pi --mode json \
            "use venv_tracelens for all commands and tool calls. Run quality LLM eval skill on $OUTPUT_DIR with reference $reference_dir for test case $id mode=$COMPARISON_SCOPE. Write results to $CASE_RESULTS/quality_llm_results.csv"
...
if [[ "$SKIP_POST_PROCESSING" == "1" ]]; then
    ...
    (
        cd "$EVALS_DIR" || exit
        pi --mode json \
            "use venv_tracelens for all commands and tool calls. Run eval post processing on results_root=$RESULTS_ROOT suite=$SUITE_NAME test_traces_csv=$TEST_TRACES_CSV report_dir=$REPORT_DIR container=${CONTAINER:-} $NODE_LABEL $RUNTIME_LABEL"
