            ...
            while [ "$agent_success" = false ] && [ "$attempts" -lt 3 ]; do
                ...
                (
                    ...
                    timeout 1800 agent --model "$AGENT_MODEL" --print --force --trust --output-format stream-json \
                        "Follow the semantic-comparison-agent workflow (TraceLens/Agent/Analysis/.cursor/agents/semantic-comparison-agent.md). Run it on trace A=$abs_a (platform $platform_a) and trace B=$abs_b (platform $platform_b), output to $OUTPUT_DIR. Run only through Step 3 'Generate TraceDiff Output' so that $OUTPUT_DIR/tracediff_output/diff_stats.csv is produced; do NOT run the full analysis orchestrator or any downstream report/category steps."
