...
from TraceLens.PerfModel.perf_model import GEMM, gemmologist_dtype_map, SDPA


def main():
    parser = argparse.ArgumentParser(
        description="Simulate GEMM or SDPA operation time using perf_model"
    )
    parser.add_argument(
        "--op",
        choices=["gemm", "sdpa"],
        required=True,
        help="Operation type: gemm or sdpa",
    )
    ...
    parser.add_argument(
        ...
        help="Data type (e.g., fp32, bf16, fp16, int8)",
    )
    parser.add_argument(
        "--arch_name", type=str, required=True, help="Architecture name"
    )
    parser.add_argument("--freq_mhz", type=int, required=True, help="Frequency in MHz")
    parser.add_argument(
        "--mem_bw_gbps", type=int, required=True, help="Memory bandwidth in GBps"
    )
    parser.add_argument(
        "--l1_bw_gbps", type=int, required=True, help="L1 bandwidth in GBps"
    )
    parser.add_argument(
        "--num_cus", type=int, required=True, help="Number of compute units"
    )
    parser.add_argument(
        "--gemm_units_per_cu", type=int, default=4, help="Number of gemm units per CU"
    )
    parser.add_argument(
        "--backward", action="store_true", help="Simulate backward pass"
    )
    parser.add_argument(
