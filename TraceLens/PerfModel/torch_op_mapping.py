...
def categorize_torch_op(row):
    ...
    Note: SSM, MoE_comm, RoPE, and CrossEntropy categories are
    categorization-only (no perf model in op_to_perf_model_class_map).
    They surface in the report with timing but without GFLOPS or TB/s.
    """
    ...
    if row["name"] in dict_cat2names["SDPA"]:
        ...
        return "MoE_comm_bwd"
    elif row["name"] in ["FusedRoPEFunc"]:
        return "RoPE_fwd"
    elif row["name"] in ["FusedRoPEFuncBackward"]:
        return "RoPE_bwd"
    elif row["name"] in ["CrossEntropyFunction"]:
