...
def read_xla_dump(file: str) -> dict:
    return_value={}
    replica_groups={}
    print(f"Reading collective events from the xla dump file {file}")
    for event_key,event_val in events_map.items():
        #print(event_key)
        for line in open(file, "r"):
            if True:
                re_pattern = re.compile(f"value:.*({event_key}[.0-9remat]*)"+"[{ ].*size=([0-9]*),.*( [sbfp3214568nuzempred]*)\[")
                ...
                if "replica_groups" in line:
                    #print(line)
                    re_pattern = re.compile(f"from instruction: %({event_key}[.0-9remat]*)"+"[{ ].*replica_groups=([0-9,\[\]<=\{\}]*)")
                    collective_name=(re_pattern.search(line))
                ...
                if "source_target_pairs" in line:
                    re_pattern = re.compile(f"from instruction: %({event_key}[.0-9remat]*)"+"[{ ].*source_target_pairs=([0-9,\[\]<=\{\}]*)")
                    collective_name=(re_pattern.search(line))
                    if collective_name is not None:
                        replica_groups=dict_add(replica_groups,collective_name.group(1),collective_name.group(2))
    #print(return_value)
...
def combine_xla_trace(coll_dict: dict,rccl_dict: dict, wait_rccl_dict: dict, roofline_enable: bool) -> dict:
    ...
    size_dict={"bf16":2,"f32":4,"s32":4,"fp16":2,"f64":8,"int32":4,"int16":2,"f8e4m3fnuz":1,"f8e5m2fnuz":1,"pred":4}
    name_dict={"bf16":"bfloat16","f32":"float","s32":"int32","f8e4m3fnuz":"fp8_e4m3","f8e5m2fnuz":"fp8_e5m2","pred":"f32"}
    command_dict={"all-gather":"all_gather_perf","all-reduce":"all_reduce_perf","reduce-scatter":"reduce_scatter_perf","all-to-all":"alltoall_perf","collective-permute":"sendrecv_perf"}
    full_dict={}
    #print_summary+=(k+","+message_size+","+v[2]+","+len(rccl_dict[k])+","+np.mean(rccl_dict[k])+","+latency+"\n")
    print_summary="collective,size,dtype,#calls,mean_latency(us),rccl_test_latency(us)\n"
    #print(coll_dict)
    for k,v in coll_dict.items():
        if not(k in rccl_dict.keys()):
            print(f'{k} found in xla dump but not in trace')
            continue
        #print(k,v)
        full_dict[k]={}
        ...
        assert int(num_groups*group_size)==8,"total number of gpus in replica group is not 8"
        ...
        message=f'mpirun -np {group_size} /home/devashah@amd.com/rccl_test_2/rccl-tests/build/{commandname} -b {message_size} -e {message_size} -f 2 -g 1 -c 0 -d {name_dict.get(v[2],"float")}'
        #message=f'/home/devashah@amd.com/rccl_test_2/rccl-tests/build/{commandname} -b {message_size} -e {message_size} -f 2 -g 1 -t {group_size} -c 0 -d {name_dict.get(v[2],"float")}'
        latency,algbw,busbw="NA","NA","NA"
        if roofline_enable:
        ...
        print_summary+=str(f'{k},{message_size},{v[2]},{len(rccl_dict[k])},{np.mean(rccl_dict[k]):.0f},{latency}\n')
        #print_summary+=(k+","+str(message_size)+","+v[2]+","+str(len(rccl_dict[k]))+","+str(np.mean(rccl_dict[k]))+","+str(latency)+"\n")
    ...
    print(print_summary)
    if not roofline_enable:
        return
    print("######### \n \n Roofline analysis \n")
    for collective in ["all-gather","all-reduce","all-to-all","reduce-scatter","collective-permute"]:
        data = []
        bw=[]
        labels=[]
        sizes=[]
        ...
        print(f"Projection Collective type {collective}, Pure collective time {pure_total_runtime/1000:.0f}ms, Total collective time {total_runtime/1000:.0f}ms, Projected total runtime {projected_runtime/1000:.0f}ms")
        fig, ax = plt.subplots()
        ...
        plt.legend()
        plt.title(f'Collective {collective} Runtime: {total_runtime/8000:.0f}ms, Projected: {projected_runtime/8000:.0f}ms')
        #plt.xlabel('Message size,')
        plt.ylabel('BW (GB/sec)')
        plt.savefig(f'rccl_plots/{collective}.pdf', bbox_inches="tight", dpi=300)
...
def read_profile(traceevents: list) -> dict:
    ...
    #for i in range(0,8):
    #    print("events",len(merged_collective_events[i]))      
    for j in range(0,queue_len): 
        ...
        for i in range(0,num_gpus):
            ...
            if c_name==name:
                ...
                print("name mismatch",c_name,name)
    print("\n\nCollectives found in the trace file",rccl.keys(),"\n")
...
if __name__ == "__main__":
    ...
    if Path(xla_dump).is_dir():
        print(f"Detecting XLA module from directory {xla_dump}", file=sys.stderr)
        xla_dump = detect_xla_file(xla_dump)
    ...
    print("\n Unique hlo_ops found in the xla dump \n")
    ...
    xplane_files=glob(args.xplane_dir+"/*.xplane.pb")
    print(args.xplane_dir+"/*.xplane.pb")
    print(xplane_files)
    assert len(xplane_files)==1, "Only one xplane.pb file in a folder"
    # Get the trace events dictionary from the xplane.pb file
    from tensorboard_plugin_profile.convert import raw_to_tool_data as convert
    result, _ = convert.xspace_to_tool_data([xplane_files[0]], "trace_viewer@^", {})
    result = result.decode("utf-8") # we get bytes back from the call above
    with open(xplane_files[0].replace("pb", "processed.json"), 'w') as writefile:
    ...
    rccl_dict, wait_rccl_dict=read_profile(traceevents)
    #sys.exit()
    combine_xla_trace(messages_combine,rccl_dict, wait_rccl_dict,args.roofline)
