...
- **Type**: `operations[i].efficiency.bound_type` with a `-bound` suffix (e.g., `memory-bound`, `compute-bound`). Do NOT use `classification.gemm_type` here — that field distinguishes quantized vs regular, not the compute/memory bound type.
...
- **Trace 2 Time (ms)**: `Kernel Time (µs)_trace2_sum / 1000` from the aligned row in `category_data/gemm_ops.csv`.
