...
def run_analysis_background(job_id: str, config: Dict):
    ...
    try:
        ...
        analysis_jobs[job_id]['progress'] = f'Running ({len(active_jobs)}/{MAX_CONCURRENT_JOBS} slots in use)...'
        ...
        if success:
            ...
            analysis_jobs[job_id]['report_path'] = str(analyzer.output_dir / 'analysis_summary.md')
        else:
        ...
        update_queue_positions()
        print(f"\n{'='*70}")
        print(f"Job {job_id} finished. Active jobs: {len(active_jobs)}/{MAX_CONCURRENT_JOBS}")
        print(f"{'='*70}\n")
...
def list_reports():
    ...
    for base_dir in report_dirs:
        ...
        for item in base_dir.iterdir():
            if item.is_dir() and (item.name.endswith('_Report') or item.name == 'web_reports'):
                    ...
                    if subdir.is_dir():
                        ...
                        if summary.exists():
                            ...
                            reports.append({
                                ...
                                'date_str': datetime.fromtimestamp(summary.stat().st_mtime).strftime('%Y-%m-%d %H:%M:%S')
                            })
...
def view_report(report_path: str):
    ...
    markdown_content = re.sub(r'!\[[^\]]*\]\(([^)]+\.png)\)', replace_image_path, markdown_content)
