...
def analyze_lca_differences(
    ...
    if len(actual_sources) != 2:
        raise ValueError(
            f"Expected 2 sources, found {len(actual_sources)}: {actual_sources}"
        )

    # Validate source values exist
    if baseline_source not in actual_sources:
        raise ValueError(
            f"Baseline source '{baseline_source}' not found. Available: {actual_sources}"
        )
    if target_source not in actual_sources:
        raise ValueError(
            f"Target source '{target_source}' not found. Available: {actual_sources}"
        )
...
def print_lca_warnings(results: Dict, baseline_name: str, target_name: str):
    ...
    if baseline_stats["lca_ids"]:
        ...
        print(
            f"   IDs: {baseline_stats['lca_ids'][:10]}{'...' if len(baseline_stats['lca_ids']) > 10 else ''}"
        )
        print(
            f"   Kernels: {baseline_stats['num_kernels']} | "
    ...
    if target_stats["lca_ids"]:
        ...
        print(
            f"   IDs: {target_stats['lca_ids'][:10]}{'...' if len(target_stats['lca_ids']) > 10 else ''}"
        )
        print(
            f"   Kernels: {target_stats['num_kernels']} | "
...
def save_lca_results(
    results: Dict, output_dir: Path, baseline_name: str, target_name: str
):
    ...
    summary = pd.DataFrame(
        {
            ...
            "Unique LCAs": [
                len(baseline_stats["lca_ids"]),
                len(target_stats["lca_ids"]),
            ],
            "Num Kernels": [baseline_stats["num_kernels"], target_stats["num_kernels"]],
            "Runtime (ms)": [
                baseline_stats["cumulative_runtime"],
                target_stats["cumulative_runtime"],
            ],
            "Percentage (%)": [
                baseline_stats["percentage_of_total"],
                target_stats["percentage_of_total"],
            ],
        }
    )
...
def analyze_multi_module_lcas(
    ...
    for source, display_name in [
        ...
        lca_groups = (
            trace_df.groupby("lowest_common_ancestor_id")
            .agg(
                {
        ...
        if len(multi_module) > 0:
            ...
            details_df = trace_df[
                trace_df["lowest_common_ancestor_id"].isin(multi_lca_ids)
            ].copy()
            ...
            for lca_id in multi_lca_ids:
                ...
                modules = (
                    lca_data.groupby("nn_module_parent")
                    .agg({"kernel_time": "sum", "name": "count"})
                    .reset_index()
                )
        ...
        results[source] = {
            ...
            "total_kernels": (
                multi_module["num_kernels"].sum() if len(multi_module) > 0 else 0
            ),
...
def print_multi_module_warnings(results: Dict, baseline_name: str, target_name: str):
    ...
    if baseline_stats["num_multi_module_lcas"] > 0:
        ...
        print(
            f"   Multi-module LCAs: {baseline_stats['num_multi_module_lcas']} | "
        ...
        for _, row in top5.iterrows():
            print(
                f"     LCA {row['lca_id']}: {row['num_modules']} modules, "
    ...
    if target_stats["num_multi_module_lcas"] > 0:
        ...
        print(
            f"   Multi-module LCAs: {target_stats['num_multi_module_lcas']} | "
...
def prepare_cleaned_diff_data(
    ...
    multi_baseline = (
        set(multi_module_results["baseline_stats"]["summary_df"]["lca_id"].tolist())
        if multi_module_results["baseline_stats"]["num_multi_module_lcas"] > 0
        else set()
    )
    multi_target = (
        set(multi_module_results["target_stats"]["summary_df"]["lca_id"].tolist())
        if multi_module_results["target_stats"]["num_multi_module_lcas"] > 0
        else set()
    )
    ...
    # Unique LCAs
    df.loc[
        (df["source"] == baseline_source)
        & (df["lowest_common_ancestor_id"].isin(unique_baseline)),
        "lowest_common_ancestor_id",
    ] = -1  # Special ID for unique baseline
    df.loc[
        (df["source"] == baseline_source) & (df["lowest_common_ancestor_id"] == -1),
        "_exclude",
    ] = True

    df.loc[
        (df["source"] == target_source)
        & (df["lowest_common_ancestor_id"].isin(unique_target)),
        "lowest_common_ancestor_id",
    ] = -2  # Special ID for unique target
    df.loc[
        (df["source"] == target_source) & (df["lowest_common_ancestor_id"] == -2),
        "_exclude",
    ] = True
    ...
    df.loc[
        (df["source"] == baseline_source)
        & (df["lowest_common_ancestor_id"].isin(multi_baseline)),
        "lowest_common_ancestor_id",
    ] = -3  # Special ID for multi-module baseline
    df.loc[
        (df["source"] == baseline_source) & (df["lowest_common_ancestor_id"] == -3),
        "_exclude",
    ] = True

    df.loc[
        (df["source"] == target_source)
        & (df["lowest_common_ancestor_id"].isin(multi_target)),
        "lowest_common_ancestor_id",
    ] = -4  # Special ID for multi-module target
    df.loc[
        (df["source"] == target_source) & (df["lowest_common_ancestor_id"] == -4),
        "_exclude",
    ] = True
...
def analyze_problematic_groups(df_cleaned: pd.DataFrame) -> Dict[str, Any]:
    ...
    grouped = (
        df_cleaned.groupby("lowest_common_ancestor_id")
        .agg(
            {
...
def save_problematic_results(results: Dict, output_dir: Path):
    ...
    if "nn_stacks" in prob_df.columns:
        prob_df["nn_stacks"] = prob_df["nn_stacks"].apply(
            lambda x: " | ".join(map(str, x))
        )
    if "nn_modules" in prob_df.columns:
        prob_df["nn_modules"] = prob_df["nn_modules"].apply(
            lambda x: ", ".join(map(str, x))
        )
...
def calculate_lca_optimization_opportunities(
    ...
    if baseline_source not in sources:
        raise ValueError(
            f"Baseline source '{baseline_source}' not found in data. Available: {sources}"
        )
    if target_source not in sources:
        raise ValueError(
            f"Target source '{target_source}' not found in data. Available: {sources}"
        )

    # Validate: Check for LCAs only in one trace
    baseline_lcas = set(
        df_with_corrections[df_with_corrections["source"] == baseline_source][
            "lowest_common_ancestor_id"
        ].unique()
    )
    target_lcas = set(
        df_with_corrections[df_with_corrections["source"] == target_source][
            "lowest_common_ancestor_id"
        ].unique()
    )
    ...
    if only_in_baseline or only_in_target:
        ...
        error_msg += (
            "These should have been filtered out in prepare_cleaned_diff_data()"
        )
        raise ValueError(error_msg)

    print(
        f"✓ Data validation passed: All {len(baseline_lcas)} LCAs present in both traces\n"
    )

    # Group by LCA and source to get totals per trace
    lca_by_source = (
        df_with_corrections.groupby(["lowest_common_ancestor_id", "source"])
        .agg(
            {
                "kernel_time": "sum",
    ...
    lca_pivot = lca_by_source.pivot(
        index="lowest_common_ancestor_id", columns="source", values="kernel_time"
    ).reset_index()

    # Get corrected_nn_parent (same for both traces)
    lca_modules = (
        df_with_corrections.groupby("lowest_common_ancestor_id")["corrected_nn_parent"]
        .first()
        .reset_index()
    )
    ...
    lca_pivot = lca_pivot.rename(
        columns={baseline_source: "baseline_time", target_source: "target_time"}
    )

    # Calculate predicted optimized time (min of both traces)
    lca_pivot["predicted_optimized"] = lca_pivot[["baseline_time", "target_time"]].min(
        axis=1
    )

    # Calculate opportunity (only for baseline)
    lca_pivot["opportunity"] = (
        lca_pivot["baseline_time"] - lca_pivot["predicted_optimized"]
    )
    ...
    print(
        f"   Total opportunity: {total_opportunity:.2f} ms ({total_opportunity/baseline_total*100:.2f}%)"
    )
    ...
    for idx, row in top10.iterrows():
        print(
            f"   LCA {row['lowest_common_ancestor_id']} ({row['corrected_nn_parent']}): "
...
def save_lca_optimization_opportunities(
    ...
    if df_with_corrections is not None:
        ...
        baseline_df = df_with_corrections[
            df_with_corrections["source"] == baseline_source
        ].copy()
        lca_baseline_info = (
            baseline_df.groupby("lowest_common_ancestor_id")
            .agg(
                {
                    "name": lambda x: "***".join(sorted(set(x))),  # kernel names
                    "cpu_op_name": lambda x: (
                        "***".join(sorted(set(x)))
                        if "cpu_op_name" in baseline_df.columns
                        else ""
                    ),  # cpu op names
                }
            )
        ...
        # Extract from target
        target_df = df_with_corrections[
            df_with_corrections["source"] == target_source
        ].copy()
        lca_target_info = (
            target_df.groupby("lowest_common_ancestor_id")
            .agg(
                {
                    "name": lambda x: "***".join(sorted(set(x))),  # kernel names
                    "cpu_op_name": lambda x: (
                        "***".join(sorted(set(x)))
                        if "cpu_op_name" in target_df.columns
                        else ""
                    ),  # cpu op names
                }
            )
        ...
        lca_opportunities = lca_opportunities.merge(
            lca_baseline_info, on="lowest_common_ancestor_id", how="left"
        )
        lca_opportunities = lca_opportunities.merge(
            lca_target_info, on="lowest_common_ancestor_id", how="left"
        )

        # Save updated opportunities with all info
        opp_with_kernels_path = (
            output_dir / "lca_optimization_opportunities_with_kernels.csv"
        )
        ...
        kernel_summary = (
            lca_opportunities.groupby("baseline_kernel_names")
            .agg(
                {
                    ...
                    "target_kernel_names": lambda x: "***".join(
                        sorted(set("***".join(x.dropna()).split("***")))
                    ),
                    "baseline_cpu_op_names": lambda x: "***".join(
                        sorted(set("***".join(x.dropna()).split("***")))
                    ),
                    "target_cpu_op_names": lambda x: "***".join(
                        sorted(set("***".join(x.dropna()).split("***")))
                    ),
                }
            )
    ...
    module_summary = (
        lca_opportunities.groupby("corrected_nn_parent")
        .agg(
            {
                "lowest_common_ancestor_id": "count",
                "baseline_time": "sum",
    ...
    if "baseline_kernel_names" in lca_opportunities.columns:
        module_agg = (
            ...
            .agg(
                {
                    "baseline_kernel_names": lambda x: "***".join(
                        sorted(set("***".join(x.dropna()).split("***")))
                    ),
                    "target_kernel_names": lambda x: "***".join(
                        sorted(set("***".join(x.dropna()).split("***")))
        ...
        module_summary = module_summary.merge(
            module_agg, on="corrected_nn_parent", how="left"
        )
