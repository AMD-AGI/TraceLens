...
class JarvisAnalyzer:
    ...
    * NEGATIVE gap = Baseline FASTER (took less time)
    ...
    '''
        ...
        try:
            response = self.client_anthropic.messages.create(
                ...
                temperature=0,
                messages=[
                    {"role": "system", "content": """You are helping to aggregate different sources of a GPU workload analysis and expand the projection table with relevant comments."""},
    ...
    def _analyze_tracelens_diff_lcas(self):
        ...
        try:
            from tracelens_diff_analyzer import (
                analyze_lca_differences, print_lca_warnings, save_lca_results,
                analyze_multi_module_lcas, print_multi_module_warnings, save_multi_module_results,
                prepare_cleaned_diff_data, analyze_problematic_groups, save_problematic_results,
                calculate_lca_optimization_opportunities, save_lca_optimization_opportunities
            )
            ...
            save_multi_module_results(multi_results, output_dir, self.gpu2_name, self.gpu1_name)
            ...
            save_lca_optimization_opportunities(
                ...
                self.gpu2_name, 
                self.gpu1_name,
                df_with_corrections=problematic_results['df_with_corrections']  # ADD this
            )
        ...
        try:
            ...
            if self.generate_plots:
                ...
                for agg_over in ["op category", "parent_module"]:
                    ...
                    baseline_df = self.gpu2_data.get('ops_unique_args') # can be ops_summary
                    target_df = self.gpu1_data.get('ops_unique_args')
                    
                    if baseline_df is not None and target_df is not None:
                        print(f"  Generating cumulative progression chart for '{agg_over}'...")
                        
                        # Create modified dataframes with unmatched categories
                        baseline_df_modified, target_df_modified, unmatched_info = self._handle_unmatched_categories(
                        ...
                        if unmatched_info['baseline_unmatched'] or unmatched_info['target_unmatched']:
                            print(f"\n  ℹ️  Unmatched categories for '{agg_over}':")
                            if unmatched_info['baseline_unmatched']:
                                print(f"    Baseline only: {unmatched_info['baseline_unmatched']}")
                            if unmatched_info['target_unmatched']:
                                print(f"    Target only: {unmatched_info['target_unmatched']}")
                        ...
                        table_path = chart_gen.generate_optimization_opportunities_table(
                            baseline_df=baseline_df_modified,
                            target_df=target_df_modified,
                            time_column='total_direct_kernel_time_sum',
                            category_column=agg_over,
            ...
            if self.gpu1_data and self.gpu2_data:
                ...
                print(f"✓ Unified comparison markdown with projections: {comparison_md_path}")
                ...
                if comparison_md_path.exists():
                    ...
                    try:
                        ...
                        if comments and len(comments) == len(df):
                            ...
                            print(f"✓ Expanded table with markdown context: {cumulative_table_expanded}")

                            saved_png_path = "cumulative_projection_" + final_table_agg + ".png"
                            html_path = visualizations.generate_interactive_optimization_html(cumulative_table_expanded, saved_png_path)
                            if html_path:
                                plot_paths.append(html_path)
                                print(f"✓ Generated interactive HTML: {html_path}")
                        else:
                            print(f"⚠️  Comment count mismatch: got {len(comments) if comments else 0}, expected {len(df)}\nComments:{comments}")
                    ...
                    print(f"⚠️  Markdown comparison not found at {comparison_md_path}, skipping table expansion")
    ...
    def _handle_unmatched_categories(
        ...
        baseline_df[category_column] = baseline_df[category_column].replace('NA', 'Root')
        ...
        if baseline_only:
            baseline_df.loc[baseline_df[category_column].isin(baseline_only), category_column] = 'Unmatched'
        
        if target_only:
            target_df.loc[target_df[category_column].isin(target_only), category_column] = 'Unmatched'
    ...
    def _generate_tracelens_diff_report(self):
        ...
        try:
            ...
            if diff_summary_exists and pruned_summary_exists:
                ...
                if is_interactive:
                    response = input("Do you want to (o)verwrite or (u)se existing? [o/u]: ").strip().lower()
                else:
...
def main():
    ...
    # Required arguments
    parser.add_argument('--target-gpu-kineto', required=True, 
                       help='Target kineto trace (local path or URL)')
    parser.add_argument('--target-gpu-et', required=False, 
                       help='Target execution trace (local path or URL, required for critical path analysis)')
    parser.add_argument('--baseline-gpu-kineto', required=True, 
                       help='Baseline kineto trace (local path or URL)')
    parser.add_argument('--baseline-gpu-et', required=False, 
    ...
    if use_critical_path and (not args.target_gpu_et or not args.baseline_gpu_et):
        print("⚠️  Warning: Execution traces (--target-gpu-et, --baseline-gpu-et) are required for critical path analysis")
