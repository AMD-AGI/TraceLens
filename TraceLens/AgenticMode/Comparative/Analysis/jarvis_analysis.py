...
class JarvisAnalyzer:
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            if self.generate_plots:
                ...
                if ai_analysis:
                    cumulative_plot = (
                        plotter.generate_cumulative_optimization_progression_chart(
                            ai_analysis
                        )
                    )
                    if cumulative_plot:
                    ...
                    cumulative_table = (
                        plotter.generate_optimization_opportunities_table(ai_analysis)
                    )
                    if cumulative_table:
                        ...
                        try:
                            ...
                            if comments and len(comments) == len(df):
                                ...
                                html_path = visualizations.generate_interactive_optimization_html(
                                    cumulative_table_expanded
                                )
                                if html_path:
            ...
            report_gen = ReportGenerator(
                self.output_dir, self.gpu1_name, self.gpu2_name, self.use_critical_path
            )
            ...
            report_path = report_gen.generate_report(
                ai_analysis, executive_summary, plot_paths, baseline_total, target_total
            )
        ...
        try:
            response = self.client_anthropic.messages.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": """You are helping to aggregate different sources of a GPU workload analysis.""",
                    },
    ...
    def _run_ai_analysis(self) -> Optional[str]:
        ...
        gemm_data = data_extractors.extract_gemm_data(
            self.gpu1_data, self.gpu2_data, self.gpu1_name, self.gpu2_name
        )
        conv_data = data_extractors.extract_conv_data(
            self.gpu1_data, self.gpu2_data, self.gpu1_name, self.gpu2_name
        )
        ops_summary = data_extractors.extract_ops_summary(
            self.gpu1_data, self.gpu2_data, self.gpu1_name, self.gpu2_name
        )
        overall_data = data_extractors.extract_overall_data(
            self.gpu1_data, self.gpu2_data
        )

        detailed_comparison = data_extractors.extract_detailed_comparison(
            self.gpu1_data, self.gpu2_data
        )

        ops_unique_args = data_extractors.extract_ops_unique_args_comparison(
            self.gpu1_data, self.gpu2_data
        )

        ops_summary_by_category = data_extractors.extract_ops_summary_by_category(
            self.gpu1_data, self.gpu2_data
        )
        ...
        if (
            "ops_summary_by_category" in self.gpu2_data
            and "ops_summary_by_category" in self.gpu1_data
        ):
            ...
            if not baseline_cats.empty and not target_cats.empty:
                ...
                # Calculate gaps
                merged["gap_ms"] = (
                    merged["total_direct_kernel_time_ms_baseline"]
                    - merged["total_direct_kernel_time_ms_target"]
                )
                ...
                for _, row in slow_cats_df.iterrows():
                    ...
                    if category in slow_categories:
                        print(
                            f"  Detected slow category: {category} (gap: {gap_ms:.2f}ms)"
                        )
                        # Extract top 10 operations for this category
                        ops_details = (
                            data_extractors.extract_detailed_operations_for_category(
                        ...
                        if ops_details:
                            ...
                            print(
                                f"    Extracted {len(ops_details)} detailed operations for {category}"
                            )
        ...
        if not comparison_md_path.exists():
            raise FileNotFoundError(
                f"Comparison markdown not found: {comparison_md_path}. This is required for LLM analysis."
            )
        ...
        print(
            f"✓ Loaded comparison markdown: {comparison_md_path} ({len(markdown_data_section)} chars)"
        )
...
def main():
    ...
    # Required arguments
    parser.add_argument(
        "--gpu1-kineto", required=True, help="GPU1 kineto trace (local path or URL)"
    )
    parser.add_argument(
        "--gpu1-et",
        required=False,
    ...
    parser.add_argument(
        "--gpu2-kineto", required=True, help="GPU2 kineto trace (local path or URL)"
    )
    parser.add_argument(
        "--gpu2-et",
        required=False,
    ...
    if use_critical_path and (not args.gpu1_et or not args.gpu2_et):
        print(
            "⚠️  Warning: Execution traces (--gpu1-et, --gpu2-et) are required for critical path analysis"
        )
        print(
