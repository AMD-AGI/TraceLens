    ...
    def generate_comparison_markdown(
        ...
        if all_sheets:
            ...
            if projection_table_paths:
                markdown_parts.append(
                    f"{len(all_sheets) + 1}. [Optimization Projections](#optimization-projections)"
                )
            markdown_parts.append("\n")
            markdown_parts.append("=" * 80)
            markdown_parts.append("\n\n")
        ...
        if projection_table_paths:
            ...
            markdown_parts.append("\n")
            markdown_parts.append(
                f"**Projection:** Optimized {gpu1_name} performance based on {gpu2_name} target\n\n"
            )
            markdown_parts.append(
                "These tables show the potential performance improvement for each category:\n"
            )
            markdown_parts.append("- **Current Time:** Baseline performance\n")
            markdown_parts.append(
                "- **Projected Optimized Time:** Best achievable performance (min of baseline/target)\n"
            )
            markdown_parts.append(
                "- **Potential Gain:** Time savings from optimization\n"
            )
            markdown_parts.append(
            ...
            for category_name, table_path in sorted(projection_table_paths.items()):
                ...
                try:
                    ...
                    markdown_parts.append(
                        f"**Total Potential Gain ({category_name}):** {total_gain:.2f} ms ({total_impact:.2f}% of baseline)\n\n"
                    )

                except Exception as e:
                    markdown_parts.append(
                        f"*Error loading projection table for {category_name}: {e}*\n\n"
                    )
        ...
        markdown_parts.append("---\n")
        markdown_parts.append(
            f"*Comparison: {gpu1_name} (Baseline) vs {gpu2_name} (Target)*\n"
