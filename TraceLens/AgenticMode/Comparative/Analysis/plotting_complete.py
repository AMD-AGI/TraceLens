...
class JarvisPlotter:
    ...
    def generate_gap_analysis_plots(self) -> List[Path]:
        ...
        try:
            ...
            baseline_data, target_data, baseline_name, target_name = self._determine_baseline_target()
            ...
            if not baseline_cat.empty and not target_cat.empty:
                plot_path = self._generate_category_gap_plot(
                    baseline_cat, target_cat, baseline_name, target_name, data_source_label
                )
                if plot_path:
            ...
            if 'gpu_timeline' in baseline_data and 'gpu_timeline' in target_data:
                plot_path = self._generate_operations_gap_plot(
                    baseline_data['gpu_timeline'], target_data['gpu_timeline'],
    ...
    def generate_optimization_opportunity_plots(self) -> List[Path]:
        ...
        try:
            baseline_data, target_data, baseline_label, target_label = self._determine_baseline_target()
            
            # Enhanced Category Analysis with Gap and Potential Gains
            if 'ops_summary_by_category' in baseline_data and 'ops_summary_by_category' in target_data:
    ...
    def generate_overall_improvement_plot(self) -> Optional[Path]:
        ...
        try:
            ...
            if 'type' in baseline_timeline.columns:
                ...
                baseline_total = float(total_time_row[time_col].values[0]) if not total_time_row.empty else 0.0
            else:
                baseline_total = 0.0
            
            if 'type' in target_timeline.columns:
                total_time_row = target_timeline[target_timeline['type'] == 'total_time']
                target_total = float(total_time_row[time_col].values[0]) if not total_time_row.empty else 0.0
            else:
                target_total = 0.0
            ...
            # Create scenarios
            scenarios = ['Current State', '25% Improvement', '50% Improvement', '75% Improvement', 'Match Target']
            times = [
                baseline_total,
            ...
            improvements = [0, current_gap * 0.25, current_gap * 0.5, current_gap * 0.75, current_gap]
            colors = ['#FF6B35', '#FFB84D', '#2ECC71', '#27AE60', '#1A9B5E']
            
            bars = ax.bar(range(len(scenarios)), times, color=colors, alpha=0.85, 
                         edgecolor='black', linewidth=2)
            
            # Add Target reference line
            ax.axhline(y=target_total, color='#004E89', linestyle='--', linewidth=3, 
                      label=f'Target Reference: {target_total:.0f}ms', zorder=1)
            
            # Add annotations
            for i, (bar, time, improvement) in enumerate(zip(bars, times, improvements)):
                height = bar.get_height()
                ax.text(bar.get_x() + bar.get_width()/2, height + 300, f'{time:.0f}ms', 
                ...
                if i == 0:
                    ...
                    ax.text(bar.get_x() + bar.get_width()/2, height/2, 
                           ...
                           bbox=dict(boxstyle='round,pad=0.7', facecolor='#27AE60', alpha=0.9))
            
            ax.set_ylabel('Total Execution Time (ms)', fontsize=13, fontweight='bold')
            ax.set_title('Overall End-to-End Performance Improvement Potential\\nBaseline Optimization Roadmap', 
    ...
    def generate_optimization_opportunities_table(self, ai_analysis: Optional[str]) -> Optional[Path]:
        """
        ...
        try:
            ...
            if 'type' in baseline_timeline.columns:
                total_time_row = baseline_timeline[baseline_timeline['type'] == 'total_time']
                baseline_total_ms = float(total_time_row[time_col].values[0]) if not total_time_row.empty else 0.0
            else:
                baseline_total_ms = 0.0
            
            if 'type' in target_timeline.columns:
                total_time_row = target_timeline[target_timeline['type'] == 'total_time']
                target_total_ms = float(total_time_row[time_col].values[0]) if not total_time_row.empty else 0.0
            else:
                target_total_ms = 0.0
            
            baseline_categories = self._extract_category_breakdown(baseline_data, baseline_total_ms)
            target_categories = self._extract_category_breakdown(target_data, target_total_ms)
            ...
            for category in sorted(baseline_categories.keys()):
                ...
                impact_pct = (gain_ms / baseline_total_ms * 100) if baseline_total_ms > 0 else 0
                
                # Extract key operations from TARGET data
                extraction_category = category
                try:
                    if extraction_category:
                        ops_df = data_extractors.extract_data_by_category(extraction_category, target_data, top_ops=5)

                    
                        if ops_df is not None and not ops_df.empty:
                            key_ops = '; '.join([f"{row['name']} shape:{row['shape']} ({row['Kernel Time (µs)_sum']/1000:.1f}ms)" 
    ...
    def generate_cumulative_optimization_progression_chart(self, 
                                                          ai_analysis: Optional[str]) -> Optional[Path]:
        """
        ...
        try:
            ...
            print(f"\\n  Chart data:")
            print(f"    Baseline: {baseline_total_ms:.2f}ms, {len(baseline_categories)} categories")
            print(f"    Projection: {projection_total_ms:.2f}ms, {len(projection_categories)} categories")
            print(f"    Target: {target_total_ms:.2f}ms, {len(target_categories)} categories")
            
            # Generate the chart
            plot_path = self._generate_cumulative_chart(
                baseline_categories, projection_categories, target_categories,
                baseline_total_ms, projection_total_ms, target_total_ms
            )
    ...
    def generate_optimization_opportunities_chart(self, ai_analysis: Optional[str]) -> Optional[Path]:
        """
        ...
        try:
            ...
            opportunities = self._extract_opportunities_table(ai_analysis, baseline_total_ms)
            ...
            # Generate the chart
            plot_path = self._generate_opportunities_chart(opportunities, baseline_total_ms)
            return plot_path
    ...
    def _get_category_data(self, baseline_data: Dict, target_data: Dict) -> Tuple[pd.DataFrame, pd.DataFrame, str]:
        ...
        if self.use_critical_path:
            ...
            if baseline_cp_nodes is not None and not baseline_cp_nodes.empty and \
               target_cp_nodes is not None and not target_cp_nodes.empty:
                baseline_cat = self._extract_categories_from_cp_nodes(baseline_cp_nodes, 'baseline')
                target_cat = self._extract_categories_from_cp_nodes(target_cp_nodes, 'target')
                return baseline_cat, target_cat, "Critical Path"
            else:
                print("  Warning: Critical path data not available, falling back to ops_summary")
        ...
        return baseline_cat, target_cat, "Timeline" if not self.use_critical_path else "Timeline (Fallback)"
    
    def _extract_categories_from_cp_nodes(self, cp_nodes: pd.DataFrame, label: str) -> pd.DataFrame:
    ...
    def _generate_category_gap_plot(self, baseline_cat: pd.DataFrame, target_cat: pd.DataFrame,
                                   baseline_name: str, target_name: str, 
                                   data_source_label: str) -> Optional[Path]:
        """Generate category-level gap analysis plot"""
        try:
            if 'op category' not in baseline_cat.columns or 'total_direct_kernel_time_ms' not in baseline_cat.columns:
                return None
            
            fig, ax = plt.subplots(figsize=(14, 8))
            
            all_cats = sorted(set(baseline_cat['op category'].tolist() + target_cat['op category'].tolist()))
            ...
            for cat in all_cats:
                b_time = baseline_cat[baseline_cat['op category'] == cat]['total_direct_kernel_time_ms'].sum()
                t_time = target_cat[target_cat['op category'] == cat]['total_direct_kernel_time_ms'].sum()
            ...
            bars1 = ax.bar([i - width/2 for i in x], baseline_times, width, 
                          label=baseline_name, color='#FF6B35', alpha=0.8)
            bars2 = ax.bar([i + width/2 for i in x], target_times, width, 
                          label=target_name, color='#004E89', alpha=0.8)
            
            # Highlight gaps
            for i, gap in enumerate(gaps):
                if gap > 0:
                    ax.text(i, max(baseline_times[i], target_times[i]) + 50, 
                           f"⚠️ +{gap:.0f}ms", 
            ...
            ax.set_title(f'GAP Analysis with Potential Optimization Opportunities ({data_source_label} View)\\n'
                        f'Baseline vs Target', fontsize=13, fontweight='bold')
            ax.set_xticks(x)
            ax.set_xticklabels([cat[:20] + '...' if len(cat) > 20 else cat for cat in all_cats], 
    ...
    def _generate_operations_gap_plot(self, baseline_timeline: pd.DataFrame, 
                                     target_timeline: pd.DataFrame,
                                     baseline_name: str, target_name: str) -> Optional[Path]:
        ...
        try:
            ...
            # Get top operations
            baseline_top = baseline_timeline.nlargest(8, time_col)[[name_col, time_col]].copy()
            target_top = target_timeline.nlargest(8, time_col)[[name_col, time_col]].copy()
            all_ops = list(set(baseline_top[name_col].tolist() + target_top[name_col].tolist()))[:8]
            ...
            for op in all_ops:
                b_time = baseline_timeline[baseline_timeline[name_col] == op][time_col].sum()
                t_time = target_timeline[target_timeline[name_col] == op][time_col].sum()
            ...
            bars1 = ax.barh([i - height/2 for i in y_pos], baseline_ops_time, height, 
                           label=baseline_name, color='#FF6B35', alpha=0.8)
            bars2 = ax.barh([i + height/2 for i in y_pos], target_ops_time, height, 
                           label=target_name, color='#004E89', alpha=0.8)
            ...
            for i, gap in enumerate(op_gaps):
                ...
                if gap > 0:
                    ax.text(max_time + 20, i, f" ⚠️ +{gap:.0f}ms", 
                           va='center', fontsize=9, fontweight='bold', color='red')
                elif gap < 0:
                    ax.text(max_time + 20, i, f" ✓ {gap:.0f}ms", 
                           va='center', fontsize=9, fontweight='bold', color='green')
            
            ax.set_xlabel('Execution Time (ms)', fontsize=12, fontweight='bold')
            ax.set_title('Top Operations GAP Analysis\\nBaseline vs Target', 
                        fontsize=13, fontweight='bold')
            ax.set_yticks(y_pos)
            ax.set_yticklabels([op[:35] + '...' if len(op) > 35 else op for op in all_ops], fontsize=9)
            ax.legend(fontsize=11)
    ...
    def _generate_category_optimization_plot(self, baseline_cat: pd.DataFrame, 
                                            target_cat: pd.DataFrame,
                                            baseline_label: str, target_label: str) -> Optional[Path]:
        ...
        try:
            ...
            bars1 = ax.bar([i - 1.5*width for i in x], baseline_times, width, 
                          label=f'{baseline_label} (Current)', 
                          color='#FF6B35', alpha=0.8, edgecolor='black', linewidth=1.5)
            bars2 = ax.bar([i - 0.5*width for i in x], target_times, width, 
                          label=f'{target_label} (Reference)', 
                          color='#004E89', alpha=0.8, edgecolor='black', linewidth=1.5)
            bars3 = ax.bar([i + 0.5*width for i in x], potential_optimized, width, 
                          label=f'{baseline_label} (50% Gap Closure)', 
                          color='#2ECC71', alpha=0.8, edgecolor='black', linewidth=1.5)
            
            # Add annotations
            for i, (b_t, t_t, gap, pot_opt) in enumerate(zip(baseline_times, target_times, gaps, potential_optimized)):
                if gap > 0:
                    gap_y = max(b_t, t_t) + 200
                    ax.annotate('', xy=(i - 1.5*width, gap_y), xytext=(i - 0.5*width, gap_y),
                               arrowprops=dict(arrowstyle='<->', color='red', lw=2.5))
                    ax.text(i - width, gap_y + 100, f'GAP\\n⚠️ +{gap:.0f}ms', 
                           ha='center', va='bottom', fontsize=9, fontweight='bold', color='red',
                ...
                if improvement > 0:
                    impr_y = pot_opt + 150
                    ax.annotate('', xy=(i - 1.5*width, impr_y), xytext=(i + 0.5*width, impr_y),
                               arrowprops=dict(arrowstyle='<->', color='green', lw=2.5))
                    ax.text(i - 0.5*width, impr_y + 100, f'POTENTIAL GAIN\\n✓ -{improvement:.0f}ms', 
                           ha='center', va='bottom', fontsize=9, fontweight='bold', color='green',
                           bbox=dict(boxstyle='round,pad=0.5', facecolor='lightgreen', alpha=0.7))
            
            ax.set_ylabel('Execution Time (ms)', fontsize=13, fontweight='bold')
            ax.set_title('Category Performance Analysis: Gap Identification & Optimization Potential\\n'
                        'Baseline vs Target with 50% Gap Closure Target', 
                        fontsize=14, fontweight='bold', pad=20)
            ax.set_xticks(x)
            ax.set_xticklabels([cat[:18] + '...' if len(cat) > 18 else cat for cat in all_cats], 
            ...
            plt.tight_layout()
            opt_gain_plot = self.output_dir / "baseline_target_category_optimization_analysis.png"
            plt.savefig(str(opt_gain_plot), dpi=150, bbox_inches='tight')
            plt.close()
            print(f"    ✓ Enhanced category optimization analysis plot: {opt_gain_plot}")
    ...
    def _extract_category_breakdown(self, gpu_data: Dict, total_ms: float) -> Dict[str, float]:
        ...
        if ops_summary is not None and not ops_summary.empty:
            ...
            for idx, row in ops_summary.iterrows():
                ...
                duration = row.get('total_direct_kernel_time_ms', row.get('duration_ms', 0))
    ...
    def _extract_categories_from_critical_path(self, cp_df: pd.DataFrame, 
    ...
    def _extract_optimization_gains(self, ai_analysis: Optional[str], 
                                   baseline_categories: Dict[str, float],
                                   baseline_total_ms: float,
                                   target_total_ms: float) -> Dict[str, float]:
        ...
        for line_num, line in enumerate(lines):
            ...
            if (not in_optimization_section and 
                (('OPPORTUNITIES' in line.upper() and 'IMPACT' in line.upper()) or
                 ('HIGH-IMPACT' in line.upper() and 'OPTIMIZATION' in line.upper())) and
                not line.strip().startswith('|')):  # Not a table cell
                in_optimization_section = True
                print(f"  Found optimization opportunities section at line {line_num}: {line.strip()}")
                continue
            ...
            if in_optimization_section:
                ...
                if line.startswith('##'):
                    if 'OPTIMIZATION' not in line.upper():
                        print(f"  End of OPTIMIZATION section at line {line_num}: {line.strip()}")
                        break
                    else:
                        # Still in optimization section, just a subsection
                        print(f"  Skipping optimization subsection header at line {line_num}")
                        continue
                ...
                if '|' in line and len(line.strip()) > 5:
                    ...
                    is_header = ('priority' in line_lower and 'category' in line_lower and 'current time' in line_lower)
                    is_cumulative_row = 'cumulative impact' in line_lower or 'tier' in line_lower
                    
                    # Only process data rows
                    if len(parts) >= 7 and not is_separator and not is_header and not is_cumulative_row:
                        ...
                        potential_gain_str = parts[5].strip() if len(parts) > 5 else ""  # Potential Gain column
                        impact_pct_str = parts[6].strip() if len(parts) > 6 else ""  # % Impact column
                        ...
                        if potential_gain_str and category_name:
                            ...
                            potential_gain_str = potential_gain_str.replace('**', '').replace('*', '').replace('ms', '').replace(',', '').strip()
                            ...
                            if ms_match:
                                ...
                                if gain_ms > 0 and gain_ms < baseline_total_ms:
                                    ...
                                    print(f"    Extracted Potential Gain: {gain_ms:.2f}ms")
                                    print(f"    Calculated % Impact: {impact_pct:.2f}% (from {impact_pct_str})")
                                else:
                                    gain_ms = 0
                        ...
                        if gain_ms > 0 and category_name:
                            ...
                            category_name = re.sub(r'[\*\`#🎯⚡⚠️✓⚪]', '', category_name).strip()
                            
                            # Map to actual category
                            category = self._map_operation_to_category(category_name, baseline_categories)
                            
                            if category:
                                category_gains_ms[category] = category_gains_ms.get(category, 0) + gain_ms
                                print(f"    ✓ Mapped '{category_name}' -> '{category}': {gain_ms:.2f}ms ({impact_pct:.2f}%)")
                            else:
                                print(f"    ✗ Could not map '{category_name}' to any baseline category")
                                print(f"       Available categories: {list(baseline_categories.keys())}")
        
        print(f"\n  ===== EXTRACTION SUMMARY =====")
        print(f"  Total extracted gains: {category_gains_ms}")
        print(f"  Total optimization potential: {sum(category_gains_ms.values()):.2f}ms")
        print(f"  Potential improvement: {(sum(category_gains_ms.values()) / baseline_total_ms * 100):.2f}%")
        print(f"  ==============================\n")
        
        return category_gains_ms
    
    def _map_operation_to_category(self, operation: str, 
                                   baseline_categories: Dict[str, float]) -> Optional[str]:
        ...
        for cat_name in baseline_categories.keys():
            ...
            cat_base = cat_lower.replace(' operations', '').replace('operations', '').strip()
            op_base = operation_lower.replace(' operations', '').replace('operations', '').strip()
        ...
        keyword_map = {
            ...
            'elementwise': ['element_wise', 'element-wise', 'relu', 'sigmoid', 'add', 'mul'],
        ...
        for base_keyword, variants in keyword_map.items():
            if base_keyword in operation_lower or any(v in operation_lower for v in variants):
                # Find matching category
                for cat_name in baseline_categories.keys():
                    cat_lower = cat_name.lower()
                    if base_keyword in cat_lower or any(v in cat_lower for v in variants):
                        return cat_name
        
        return None
    
    def _extract_opportunities_table(self, ai_analysis: str, baseline_total_ms: float) -> List[Dict]:
        ...
        for line_num, line in enumerate(lines):
            ...
            if (not in_opportunities_section and 
                ('OPPORTUNITIES' in line.upper() or 'HIGH-IMPACT' in line.upper()) and 
                'IMPACT' in line.upper() and
                not line.strip().startswith('|')):
                in_opportunities_section = True
                print(f"  Found opportunities section at line {line_num}: {line.strip()}")
                continue
            
            # Stop at next major section
            if in_opportunities_section:
                if line.startswith('##') and 'OPPORTUNITIES' not in line.upper() and 'IMPACT' not in line.upper():
                ...
                if '|' in line and len(line.strip()) > 5:
                    ...
                    if len(parts) >= 7 and not is_separator and not is_header and not is_cumulative:
                        ...
                        if category and potential_gain > 0:
                            ...
                            print(f"    Added: {category} - {potential_gain:.2f}ms ({impact_pct:.2f}%)")
    ...
    def _extract_ms_value(self, text: str) -> float:
        ...
        text = text.replace('**', '').replace('*', '').replace('ms', '').replace(',', '').strip()
    ...
    def _generate_opportunities_chart(self, opportunities: List[Dict], baseline_total_ms: float) -> Optional[Path]:
        ...
        try:
            ...
            bars = ax.barh(y_pos, gains, color=colors, alpha=0.8, edgecolor='black', linewidth=1.2)
            
            # Add value labels on bars
            for i, (bar, gain, impact) in enumerate(zip(bars, gains, impacts)):
                width = bar.get_width()
                ax.text(width + max(gains) * 0.02, bar.get_y() + bar.get_height()/2,
                       f'{gain:.1f} ms\n({impact:.2f}%)',
            ...
            ax.set_title('High-Impact Optimization Opportunities', 
                        fontsize=14, fontweight='bold', pad=20)
            ...
            ax.text(0.02, 0.98, f'Cumulative Impact: {total_gain:.1f} ms ({total_impact:.2f}%)',
    ...
    def _calculate_projection(self, baseline_categories: Dict[str, float],
                             category_gains_ms: Dict[str, float]) -> Dict[str, float]:
        ...
        for cat, baseline_val in baseline_categories.items():
            ...
            if gain_ms > 0:
                ...
                print(f"    {cat}: {baseline_val:.2f}ms -> {projected_val:.2f}ms (gain: {gain_ms:.2f}ms)")
            else:
                projection[cat] = baseline_val
        
        return projection
    
    def _generate_cumulative_chart(self, baseline_cat: Dict[str, float],
                                  projection_cat: Dict[str, float],
                                  target_cat: Dict[str, float],
                                  baseline_total: float,
                                  projection_total: float,
                                  target_total: float) -> Optional[Path]:
        ...
        try:
            ...
            all_categories = sorted(set(baseline_cat.keys()) | set(projection_cat.keys()))
            ...
            for cat in all_categories:
                ...
                bars = ax.bar(x_pos, values, bar_width, label=cat, 
                             bottom=bottom_values, color=cat_colors[cat],
            ...
            ax.set_title('Cumulative Performance Optimization Projection (idle time not shown)',
            ...
            for i, total in enumerate(totals):
                ax.text(i, total + max(totals) * 0.008, f'{total:.0f}ms', 
                ...
                if i > 0:
                    ...
                    improvement_pct_active = ((baseline_active - projection_active) / baseline_active * 100) if baseline_active > 0 else 0
                    
                    # Calculate total improvement (with idle)
                    improvement_pct_total = ((baseline_active - projection_active) / baseline_total * 100) if baseline_total > 0 else 0
                    
                    ax.text(i, total + max(totals) * 0.035, 
            ...
            plt.tight_layout()
            cumulative_plot = self.output_dir / "baseline_cumulative_optimization_progression.png"
            plt.savefig(str(cumulative_plot), dpi=150, bbox_inches='tight', 
