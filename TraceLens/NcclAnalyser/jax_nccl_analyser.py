...
class JaxNcclAnalyser:
    ...
    def __init__(self, traces_dir, node_to_pb_file_mapping, world_size, node_to_xla_file_map=None):
        """
    ...
    def build_df_long(self):
        ...
        for node, node_events in self.node_to_trace_data.items():
            for event in node_events.values():
                ...
                if collective_name:
                    replica_string, replica_groups, data_bytes = self._lookup_collective_info(node, collective_name)
    ...
    def analyze_all_collectives_from_df(self, df=None):
        ...
        for collective_name in unique_collectives:
            bandwidths, bus_bandwidths, avg_bandwidth, avg_bus_bandwidth, slice_info = self._calculate_collective_bandwidth_from_df(df, collective_name)
    ...
    def analyze_collective_types_from_df(self, bandwidth_results):
        ...
        for collective_name, result in bandwidth_results.items():
            ...
            collective_types[col_type]['all_bus_bandwidths'].extend(result['bus_bandwidths'])
    ...
    def _build_node_to_xla_file_map(self):
        ...
        xla_pattern = os.path.join(self.traces_dir, "**", "xla_dumps", "*jit_train_step.*_gpu_after_optimizations.txt")
    ...
    def _calculate_bandwidth_per_replica_group(self, df, collective_name, collective_id):
        """
        ...
        for group_idx, gpu_group in enumerate(gpu_groups):
            ...
            if fastest_gpu_duration_s > 0:
                ...
                algorithmic_bandwidth_gbps = (algorithmic_bytes / (1024**3)) / fastest_gpu_duration_s
                
                # Calculate bus bandwidth using the scaler
                bus_bandwidth_scaler = self.get_bus_bandwidth_scaler(collective_name, actual_group_size)
                ...
                print(f"        Algorithmic Bandwidth: {algorithmic_bandwidth_gbps:.2f} GB/s")
                print(f"        Bus Bandwidth: {bus_bandwidth_gbps:.2f} GB/s (scaler: {bus_bandwidth_scaler:.3f})")
    ...
    def _calculate_collective_bandwidth_from_df(self, df, collective_name):
        ...
        for collective_id in unique_collective_ids:
            ...
            group_bandwidths, group_bus_bandwidths, group_details = self._calculate_bandwidth_per_replica_group(
        ...
        all_slice_bus_bandwidths = [info['slice_avg_bus_bandwidth'] for info in slice_info if 'slice_avg_bus_bandwidth' in info]
        overall_avg_bus_bandwidth = np.mean(all_slice_bus_bandwidths) if all_slice_bus_bandwidths else 0.0
        ...
        return all_slice_bandwidths, all_slice_bus_bandwidths, overall_avg_bandwidth, overall_avg_bus_bandwidth, slice_info