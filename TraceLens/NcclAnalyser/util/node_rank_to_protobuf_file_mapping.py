...
def get_node_rank_protobuf_mapping(traces_folder, pattern="*.xplane.pb"):
    ...
    if nnodes is not None:
        ...
        if found_ranks != expected_ranks:
            print(
                f"Warning: NODE_RANK count ({found_ranks}) does not match nnodes ({expected_ranks})"
            )
    ...
    for pb_file in pb_files:
        ...
        if node_name and node_name in node_to_rank_map:
            ...
            print(
                f"Warning: Found protobuf for node {node_name} but no corresponding NODE_RANK in log files"
            )
