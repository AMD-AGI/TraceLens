...
class XLACollectiveParser:
        ...
        for node, file_path in self.node_to_file_mapping.items():
            ...
            for line in lines:
                ...
                if line and re.search(
                    r'scheduling_name="[^"]*(?:all-to-all|all-gather|all-reduce|reduce-scatter|collective-permute)[^"]*"',
                    line,
                ):
                    ...
                    replica_group_size = self._calculate_replica_group_size(
                        replica_groups
                    )
                    tensor_slice = self._calculate_tensor_slice(
                        tensors,
                        split_dimension,
                        replica_groups,
                        collective_name,
    ...
    def _parse_replica_groups(self, replica_groups_str):
        ...
        if not replica_groups_str or str(replica_groups_str).strip().lower() in [
            "nan",
            "none",
            "",
        ]:
        ...
        if replica_groups_str.startswith("{{"):
            ...
            return [
                [int(x.strip()) for x in group_str.split(",")]
    ...
    def _parse_device_assignment(self, replica_groups_str):
        ...
        if transpose_match:
            transpose_perm = [
                int(x.strip()) for x in transpose_match.group(1).split(",")
            ]
    ...
    def _extract_tensor_specs(self, line):
        ...
        equals_match = re.search(
            r"=\s+(.+?)\s+(?:all-to-all|all-gather|all-reduce|reduce-scatter|collective-permute)\(",
            line,
        )
        if equals_match:
            return equals_match.group(1).strip()

        equals_fallback = re.search(
            r"=\s+(.+?)(?:\s+[a-z-]+\(|,\s+channel_id|,\s+replica_groups|$)", line
        )
        if equals_fallback:
    ...
    def _extract_output_tensor_from_tuple(self, collective_name, tensor_spec):
        ...
        if collective_name and (
            "reduce-scatter" in collective_name
            or "collective-permute" in collective_name
        ):
            return tensor_spec

        if collective_name and (
            "all-gather-start" in collective_name or "all-to-all" in collective_name
        ):
            ...
            # Handle simple tuples: (input, output)
                ...
                tensor_spec.startswith("(")
                and ", " in tensor_spec
                and tensor_spec.count("(") == 1
            ):
    ...
    def _calculate_tensor_slice(
        ...
        output_tensor = self._extract_output_tensor_from_tuple(
            collective_name, tensor_spec
        )
        ...
        for dtype_str, dimensions_str in matches:
            dimensions = [
                int(d.strip()) for d in dimensions_str.split(",") if d.strip()
            ]
            ...
            if pd.isna(split_dimension):
                ...
                split_dimension >= len(dimensions)
                or dimensions[split_dimension] % replica_group_size != 0
            ):
                continue
            else:
                sliced_dimensions = dimensions.copy()
                sliced_dimensions[split_dimension] = (
                    dimensions[split_dimension] // replica_group_size
                )
                slice_info = {
                    "dtype": dtype_str,
    ...
    def _calculate_data_bytes(self, tensor_slice):
        ...
        if tensor_slice is None:
            ...
            return sum(
                slice_info.get("bytes", 0)
                for slice_info in tensor_slice
    ...
    def _calculate_replica_group_size(self, replica_groups):
        ...
        return (
            len(replica_groups[0]) if replica_groups and len(replica_groups) > 0 else 0
        )
