...
def find_steady_state(
    ...
    *,
    ...
    """Route to the steady-state finder that fits the trace's available signals.

    - serving annotations present  -> :func:`find_steady_state_inference`
    - batch sizes inferred (no annotations) -> :func:`find_steady_state_inference_from_shapes`
    - neither -> :func:`find_steady_state_generic` (duration-based)

    All three return ``(selected_roots, regions)``.
    """
    if has_annotations:
        kwargs = {"mode": mode}
        if mode == "mixed":
            kwargs.update(CONC=CONC, OSL=OSL, R=R)
        return find_steady_state_inference(roots, num_steps=num_steps, **kwargs)
    if batch_sizes is not None:
        return find_steady_state_inference_from_shapes(
            roots, batch_sizes, num_steps=num_steps, mode=mode, max_num_seq=max_num_seq
        )
    return find_steady_state_generic(roots, num_steps=num_steps)
