...
def ancestors_of(tree, event: dict, max_depth: int = 8) -> List[dict]:
    """Enclosing spans above ``event``, nearest first.

    The depth bound is load-bearing: walk far enough and you reach the thread
    entry point, a single root spanning the trace that passes coverage uselessly.
    """
    out, current = [], event
...
def in_gap_candidates(
    index: IntervalIndex, gaps: Sequence[dict], categories: Sequence[str]
) -> List[dict]:
    """Events of the given categories sitting in inter-root gaps."""
    out = []
    for gap in gaps:
        out.extend(
            e
            for e in index.contained_in(gap, exclude_self=False)
            if e.get("cat") in categories
        )
    return out
...
@dataclass
class Probe:
    """One escalation attempt: ``applies_to`` gates it, ``run`` proposes roots."""

    name: str
    applies_to: Callable[[RootSet], bool]
    run: Callable[[RootSet], Optional[RootSet]]


def run_probes(
    root_set: RootSet,
    probes: Sequence[Probe],
    audit: Callable[[RootSet], CoverageReport],
) -> RootSet:
    """Try probes in declared order, re-measuring coverage after each.

    Stops at the first that clears the gate, recording what every attempt did to
    coverage. If none clears it, the best result is kept and graded against the
    floor as degraded or unsplittable.
    """
    attempts: List[dict] = []
    best = root_set
    for probe in probes:
        if not probe.applies_to(best):
            attempts.append({"probe": probe.name, "outcome": "skipped"})
            continue
        candidate = probe.run(best)
        if candidate is None or not candidate.roots:
            attempts.append({"probe": probe.name, "outcome": "no_candidate"})
            continue
        candidate.coverage = audit(candidate)
        before = best.coverage.covered_selected if best.coverage else 0.0
        after = candidate.coverage.covered_selected
        attempts.append(
            {
                "probe": probe.name,
                "outcome": "adopted" if after > before else "rejected",
                "coverage_before": round(before, 4),
                "coverage_after": round(after, 4),
                "n_roots": len(candidate.roots),
            }
        )
        if after > before:
            best = candidate
        if best.coverage and best.coverage.passes:
            break

    best.diagnostics["probes_run"] = attempts
    coverage = best.coverage
    if coverage and coverage.passes:
        best.status = DetectStatus.SPLITTABLE
    elif coverage and coverage.covered_selected >= COVERAGE_FLOOR:
        best.status = DetectStatus.DEGRADED
    else:
        best.status = DetectStatus.NOT_SPLITTABLE
    return best
