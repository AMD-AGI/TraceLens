...
- `--profiler-config.detailed_trace_annotation True` — enables detailed annotations in the trace (iteration boundaries, phase labels, scheduling metadata). These annotations are required by `trace_split.main` for accurate trace splitting.
...
- `ATOM_ENABLE_DETAILED_ANNOTATION: "1"` — appends the roofline aggregates (`sqsq`, `sqsk`, `sk`) to the `prefill[]` / `decode[]` step annotations that `trace_split.main` consumes. The aggregates are computed only while the profiler is running.
...
- **The window cannot be narrowed to a fixed iteration count.** The trace covers the whole profiled request phase, so it will be larger than a comparable vLLM or SGLang targeted window. Use Step 6's `trace_split.main --find-steady-state` to extract the steady-state region on the host afterwards.
...
ssh <node> "source ~/miniconda3/etc/profile.d/conda.sh && conda activate <env> && \
  cd <TraceLens_repo> && \
  python -m TraceLens.TraceUtils.trace_split.main \
