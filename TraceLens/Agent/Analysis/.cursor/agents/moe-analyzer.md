...
**Roofline-unresolved dominant MoE kernel:** a fused-MoE expert kernel (e.g. `invoke_fused_moe_kernel`) frequently carries no resolvable input dims, so its roofline efficiency is null and the analyzer emits a finding with `roofline_unresolved: true` and `impact_score: null`. This is **NOT** an empty-findings case — emit the P-item and its matching reasoning candidate exactly as in [`sub_agent_spec.md` § Roofline-unresolved findings](../utils/templates/sub_agent_spec.md#roofline-unresolved-findings): `**Impact**: Not quantifiable from trace data` (markers `low=null mid=null high=null`), Efficiency/FLOPS/Byte/Bound rendered `—`, and the `Kernel Path` column populated verbatim from `operations[i].launcher_path` (so the dominant kernel still reaches the report and downstream candidate extraction). Use the `compute` Action Prose row (profile the kernel for tile-size / occupancy tuning) since fused MoE kernels are grouped-GEMM compute kernels; do not claim a specific achieved bound that the trace did not resolve.
