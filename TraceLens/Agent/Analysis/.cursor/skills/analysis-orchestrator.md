...
4. **Analysis Mode** → `<analysis_mode>`
   ...
   - **Comparative + graph replay is routed in Step 0.5** (do not abort): collect capture folders for both traces when available. Step 0.5 routes on capture availability — graph traces **with** capture use the direct capture-folder comparison (tracediff + `--comparison_capture_folder`); graph traces **without** capture use the **semantic** path.
...
|-------|-------|
| `standalone` | none |
| `comparative` trace1 if `<capture_folder_path2>` provided | `--comparison_capture_folder <capture_folder_path2>` |
| `comparative` trace2 | none |
