...
def write_summary_report(
    ...
    for cat, mode_counts in sorted(
        ...
        affected = len(set(f.model_name for f in findings if f.category == cat))
...
def _collectible_paths(run_dir: str, max_bytes: int) -> List[Tuple[str, str]]:
    ...
    _add(
        os.path.join(split_dir, "execution_details.csv"),
        "trace_split/execution_details.csv",
    )
    _add(
        os.path.join(split_dir, "execution_details.json"),
