...
def _auto_detect_log(run_dir):
    ...
    parent = str(Path(run_dir).parent)
    candidates = sorted(
        glob.glob(os.path.join(parent, "logs", "tracelens_analysis", "*.log"))
    )
    if candidates:
        return candidates[-1]
    return None
...
def run_triage(run_dir, stream_file=None, detailed=False):
    ...
    if not os.path.isdir(run_dir):
        print(
            f"[DIAG:tracelens_agent_workflow:OUTPUT_INCOMPLETE] Run directory does not exist: {run_dir}"
        )
        sys.exit(1)

    if stream_file and not os.path.isfile(stream_file):
        print(
            f"Warning: stream file not found: {stream_file} — stream checks will be skipped"
        )
        stream_file = None
...
def write_detail_csv(findings, run_dir):
    ...
    with open(path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(
            ["DIAG Tag", "Category", "Failure Mode", "Evidence", "Remedy", "Implied By"]
        )
        for finding in findings:
            writer.writerow(
                [
...
def main():
    ...
    parser.add_argument(
        "--run-dir", default=None, help="Path to analysis_output/ directory"
    )
    parser.add_argument(
        "--session-dir",
        default=None,
    ...
    parser.add_argument(
        "--stream-file",
        default=None,
        help="Path to agent stream (.ndjson or .streamJSON)",
    )
    parser.add_argument(
        "--detailed",
        action="store_true",
