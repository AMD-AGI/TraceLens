...
def extract_critical_path_comparison(
    baseline_data: Dict, target_data: Dict, baseline_total: float
    ...
    try:
        ...
        if (
            name_col not in baseline_crit_ops.columns
            or time_col not in baseline_crit_ops.columns
        ):
        ...
        for op_name in baseline_cp_ops:
            try:
                ...
                impact_pct = (
                    (b_time / baseline_total * 100.0) if baseline_total > 0 else 0.0
                )
                ...
                cp_comparison_data.append(
                    {
                        ...
                        "target_time_ms": (
                            round(t_time, 2) if t_time is not None else None
                        ),
...
def extract_timeline_comparison(
    baseline_data: Dict, target_data: Dict, baseline_total: float
) -> Tuple[List[Dict], List[str]]:
    ...
    try:
        ...
        for _, row in baseline_ops.iterrows():
            cat_name = row.get(
                "op category", row.get("category", row.get("Category", "Unknown"))
            )
            b_time = float(
                row.get("total_direct_kernel_time_ms", row.get("total_duration_ms", 0))
            )
    ...
    try:
        if category == "elementwise":
            ...
            if ops is not None and not ops.empty:
                ...
                df["shape"] = ops.apply(
                    lambda row: extract_shape(row, category), axis=1
                )
                result.append(df)
...
def extract_gemm_data(
    gpu1_data: Dict, gpu2_data: Dict, gpu1_name: str, gpu2_name: str
) -> Dict:
    """
    ...
    try:
        ...
        if "GTOPS_first" in baseline_ops.columns:
            baseline_gflops = pd.to_numeric(
                baseline_ops["GTOPS_first"], errors="coerce"
            )
        ...
        gemm["gap_percentage"] = round(
            (
                ((baseline_total_ms - target_total_ms) / baseline_total_ms * 100)
        ...
        print(
            f"  ✓ GEMM summary: {gpu2_name}={baseline_total_ms:.2f}ms, "
    ...
    try:
        ...
        for cat in conv_categories:
            ...
            if count_col and count_col in baseline_ops.columns:
                baseline_counts = pd.to_numeric(
                    baseline_ops[count_col], errors="coerce"
                )
            ...
            cat_data["gap_percentage"] = round(
                (
                    ((baseline_time_ms - target_time_ms) / baseline_time_ms * 100)
            ...
            print(
                f"  ✓ {cat}: {gpu2_name}={baseline_time_ms:.2f}ms, "
        ...
        if baseline_time_total > 0 or target_time_total > 0:
            ...
            conv_data["gap_percentage"] = round(
                (
                    (
                        (baseline_time_total - target_time_total)
                        / baseline_time_total
                        * 100
                    )
            ...
            print(
                f"  ✓ CONV total: {gpu2_name}={baseline_time_total:.2f}ms, "
...
def extract_ops_summary(
    gpu1_data: Dict, gpu2_data: Dict, gpu1_name: str, gpu2_name: str
) -> Dict:
    """Extract operations unique to each GPU based on ops_summary_by_category"""
    ops_summary = {f"{gpu1_name}_only": [], f"{gpu2_name}_only": []}
    ...
    try:
        ...
        if (
            baseline_ops is None
            or baseline_ops.empty
            or target_ops is None
            or target_ops.empty
        ):
            return ops_summary
        ...
        if baseline_only:
            ...
            for cat in baseline_only:
                ...
                if not cat_data.empty:
                    total_time = (
                        float(cat_data[time_col].iloc[0])
                        if time_col in cat_data.columns
                        else 0
                    )
                    count = (
                        int(cat_data[count_col].iloc[0])
                        if count_col in cat_data.columns
                        else 0
                    )
            ...
            ops_summary[f"{gpu2_name}_only"].sort(
        ...
        if target_only:
            ...
            # Sort by total time descending and limit to top 10
            ops_summary[f"{gpu1_name}_only"].sort(
                key=lambda x: x["total_time_ms"], reverse=True
            )
...
def extract_overall_data(gpu1_data: Dict, gpu2_data: Dict) -> Dict:
    ...
    try:
        ...
        if baseline_timeline is not None:
            ...
            if "type" in baseline_timeline.columns:
                ...
                if not total_time_row.empty:
                    overall_data["baseline_total_time_ms"] = round(
                        float(total_time_row[time_col].values[0]), 2
                    )
                # else:
        ...
        if target_timeline is not None:
            time_col = (
                "time ms" if "time ms" in target_timeline.columns else "duration_ms"
            )
            ...
            if "type" in target_timeline.columns:
                ...
                if not total_time_row.empty:
                    overall_data["target_total_time_ms"] = round(
                        float(total_time_row[time_col].values[0]), 2
                    )
                # else:
        ...
        # Calculate gaps
        if (
            "baseline_total_time_ms" in overall_data
            and "target_total_time_ms" in overall_data
        ):
            gap = (
                overall_data["baseline_total_time_ms"]
                - overall_data["target_total_time_ms"]
            )
            overall_data["total_gap_ms"] = round(gap, 2)
            overall_data["gap_percentage"] = round(
                (
                    (gap / overall_data["baseline_total_time_ms"] * 100)
                    if overall_data["baseline_total_time_ms"] > 0
                    else 0
                ),
                2,
            )
        ...
        if baseline_ops is not None and not baseline_ops.empty:
            ...
            overall_data["baseline_categories"] = (
                baseline_ops["op category"].tolist()
                if "op category" in baseline_ops.columns
                else []
            )

        if target_ops is not None and not target_ops.empty:
            overall_data["target_category_count"] = len(target_ops)
            overall_data["target_categories"] = (
                target_ops["op category"].tolist()
                if "op category" in target_ops.columns
                else []
            )
...
def extract_ops_unique_args_comparison(
    gpu1_data: Dict, gpu2_data: Dict
) -> Optional[Dict]:
    """
    ...
    try:
        ...
        print(
            f"  Available columns in ops_unique_args: {list(baseline_timeline.columns[:10])}... (showing first 10)"
        )
        ...
        print(
            f"  Found optional columns: kernel_detail='{kernel_detail_col}', input_dims='{input_dims_col}'"
        )
        ...
        ops_comparison["top_slowdowns"] = sorted(
            slowdowns, key=lambda x: x["gap_ms"], reverse=True
        )[:10]
        ops_comparison["top_speedups"] = sorted(speedups, key=lambda x: x["gap_ms"])[
            :10
        ]
...
def extract_ops_summary_by_category(gpu1_data: Dict, gpu2_data: Dict) -> Optional[Dict]:
    ...
    """
    category_comparison = {"categories": [], "total_categories": 0}
    ...
    try:
        ...
        print(
            f"  Available columns in ops_summary_by_category: {list(baseline_cat.columns)}"
        )
        ...
        print(
            f"  Using columns: category='{cat_col}', time='{time_col}', count='{count_col}', pct='{pct_col}'"
        )
            ...
            if not target_match.empty:
                ...
                category_info = {
                    ...
                    "gap_percentage": round(
                        (gap / baseline_time * 100) if baseline_time > 0 else 0, 2
                    ),
                }
        ...
        category_comparison["categories"] = sorted(
            categories_list, key=lambda x: abs(x["gap_ms"]), reverse=True
        )
...
def extract_critical_path_categories(gpu_data: Dict) -> Dict[str, float]:
    ...
    try:
        ...
        if crit_ops is not None and not crit_ops.empty:
            ...
            if (
                "category" in crit_ops.columns
                and "total_duration_us" in crit_ops.columns
            ):
                # Aggregate by category
                ...
                print(
                    f"  Extracted {len(categories)} categories from crit_ops_unique_args"
                )
                return categories
        ...
        if cp_nodes is not None and not cp_nodes.empty:
            ...
            if time_col and "category" in cp_nodes.columns:
                ...
                print(
                    f"  Extracted {len(categories)} categories from critical_path_nodes"
                )
                return categories
...
def extract_timeline_categories_comparison(
    baseline_data: Dict, target_data: Dict, baseline_total: float, target_total: float
) -> List[Dict]:
    """
    ...
    try:
        ...
        for category in categories_to_extract:
            ...
            if not baseline_row.empty:
                ...
                target_time = (
                    float(target_row[time_col].iloc[0])
                    if not target_row.empty
                    else None
                )
                ...
                baseline_pct = (
                    (baseline_time / baseline_total * 100.0)
                    if baseline_total > 0
                    else 0.0
                )
                target_pct = (
                    (target_time / target_total * 100.0)
                    if target_time is not None and target_total > 0
                    else None
                )

                # Calculate pct_gap from unrounded percentages for accuracy
                pct_gap = (
                    round(baseline_pct - target_pct, 2)
                ...
                gpu_timeline.append(
                    {
                        ...
                        "baseline_time_ms": round(baseline_time, 2),
                        "target_time_ms": (
                            round(target_time, 2) if target_time is not None else None
                        ),
                        "gap_ms": round(gap, 2) if gap is not None else None,
                        "baseline_pct": round(baseline_pct, 2),
                        "target_pct": (
                            round(target_pct, 2) if target_pct is not None else None
                        ),
                        "pct_gap": pct_gap,
                    }
                )
...
def extract_detailed_operations_for_category(
    baseline_data: Dict, target_data: Dict, category: str, top_n: int = 10
) -> List[Dict]:
    """
    ...
    try:
        ...
        if (
            not all(col in baseline_ops.columns for col in required_cols)
            or time_col is None
        ):
        ...
        baseline_cat_ops = baseline_cat_ops.sort_values(time_col, ascending=False).head(
            top_n
        )
        ...
        for idx, row in baseline_cat_ops.iterrows():
            ...
            if "avg_duration_us" in row:
                op_details["avg_duration_ms"] = round(
                    float(row["avg_duration_us"]) / 1000.0, 2
                )
...
def extract_prefill_decode_comparison(
    baseline_data: Dict, target_data: Dict
) -> Optional[Dict]:
    """
    ...
    try:
        ...
        if not baseline_prefill.empty:
            print(
                f"  Available columns in ops_summary_prefill: {list(baseline_prefill.columns)}"
            )
        ...
        def extract_phase_data(df, phase_name):
            ...
            count_col = next(
                (col for col in count_col_names if col in df.columns), None
            )
            ...
            print(
                f"  Using columns for {phase_name}: category='{cat_col}', name='{name_col}', time='{time_col}', count='{count_col}', pct='{pct_col}'"
            )

            phase_data = {"total_time_ms": 0.0, "categories": []}
            ...
            phase_data["categories"] = sorted(
                categories_list, key=lambda x: x["time_ms"], reverse=True
            )

            return phase_data

        # Extract all four phase data
        result["baseline_prefill"] = extract_phase_data(
            baseline_prefill, "baseline_prefill"
        )
        result["baseline_decode"] = extract_phase_data(
        ...
        if baseline_prefill_time > 0 or baseline_decode_time > 0:
            result["phase_comparison"] = {
                ...
                "prefill_gap_pct": round(
                    (
                        (
                            (baseline_prefill_time - target_prefill_time)
                            / baseline_prefill_time
                            * 100
                        )
                        if baseline_prefill_time > 0
                        else 0
                    ),
                    2,
                ),
                "decode_gap_pct": round(
                    (
                        (
                            (baseline_decode_time - target_decode_time)
                            / baseline_decode_time
                            * 100
                        )
                        if baseline_decode_time > 0
                        else 0
                    ),
                    2,
                ),
        ...
        print(
            f"    Baseline prefill: {baseline_prefill_time:.2f}ms ({len(result['baseline_prefill'].get('categories', []))} categories)"
        )
        print(
            f"    Baseline decode: {baseline_decode_time:.2f}ms ({len(result['baseline_decode'].get('categories', []))} categories)"
        )
