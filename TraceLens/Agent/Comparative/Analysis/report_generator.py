...
class ReportGenerator:
        ...
        with open(self.summary_file, "w") as f:
            ...
            if plot_paths:
                ...
                if gap_chart:
                    ...
                    rel_path = (
                        gap_chart.relative_to(self.output_dir)
                        if gap_chart.is_relative_to(self.output_dir)
                        else gap_chart
                    )
                ...
                if cumulative_plot:
                    ...
                    rel_path = (
                        cumulative_plot.relative_to(self.output_dir)
                        if cumulative_plot.is_relative_to(self.output_dir)
    ...
    def _write_header(self, f):
        """Write report header"""
        analysis_mode = (
            "Critical Path Analysis" if self.use_critical_path else "Timeline Analysis"
        )

        f.write(
            f"# GAP Analysis with Potential Optimization Opportunities ({analysis_mode})\n\n"
        )
        ...
        if not self.use_critical_path:
            f.write(
                "*Note: Critical path analysis is disabled. Analysis based on timeline and operation-level data.*\n\n"
            )
    ...
    def _write_footer(self, f, baseline_total_ms: float, target_total_ms: float):
        ...
        gap_pct = (
            (gap_ms / min(baseline_total_ms, target_total_ms) * 100)
            if min(baseline_total_ms, target_total_ms) > 0
            else 0
        )

        if baseline_total_ms > target_total_ms:
            f.write(
                f"- **Gap:** {gap_ms:.2f} ms ({gap_pct:.2f}% - Baseline trailing)\n"
            )
        else:
    ...
    def embed_plot(self, plot_path: Path, title: Optional[str] = None):
        ...
        with open(self.summary_file, "a") as f:
            ...
            rel_path = (
                plot_path.relative_to(self.output_dir)
                if plot_path.is_relative_to(self.output_dir)
                else plot_path
            )
