...
class MarkdownGenerator:
    ...
    def __init__(
        ...
        self.included_sheets = (
            included_sheets  # If set, use whitelist instead of blacklist
        )
    ...
    def generate_markdown(
        ...
        max_rows_ops_unique_args: int = 20,
    ) -> str:
        """
        ...
        if self.included_sheets:
            ...
            included_sheets = [
                name
                for name in result_dfs.keys()
                if name in self.included_sheets
                and isinstance(result_dfs[name], pd.DataFrame)
                and not result_dfs[name].empty
            ]
        else:
        ...
        if included_sheets:
            ...
            for i, sheet_name in enumerate(included_sheets, 1):
                markdown_parts.append(
                    f"{i}. [{sheet_name}](#{sheet_name.lower().replace(' ', '-').replace('_', '-')})"
                )
            markdown_parts.append("\n")
            markdown_parts.append("=" * 80)
            markdown_parts.append("\n\n")
        ...
        for sheet_name in included_sheets:
            ...
            if len(df) > row_limit:
                markdown_parts.append(
                    f"*Showing first {row_limit} of {len(df)} rows*\n\n"
                )
            ...
            try:
                ...
                for col in df_formatted.select_dtypes(include=["object"]).columns:
                    df_formatted[col] = df_formatted[col].apply(
                        lambda x: (
                            (str(x)[:100] + "...")
                            if isinstance(x, str) and len(str(x)) > 100
                            else x
                        )
                    )

                # Format numeric columns
                for col in df_formatted.select_dtypes(
                ...
                if sheet_name == "ops_unique_args":
                    ...
                    if cols_to_drop:
                        ...
                        markdown_parts.append(
                            f"*Note: Dropped {len(cols_to_drop)} verbose columns: {', '.join(cols_to_drop[:3])}{'...' if len(cols_to_drop) > 3 else ''}*\n\n"
                        )
        ...
        markdown_parts.append("---\n")
        markdown_parts.append(
            f"*Generated for {gpu_name} | Excluded sheets: {', '.join(self.excluded_sheets)}*\n"
        )
    ...
    def generate_comparison_markdown(
        ...
        max_rows_ops_unique_args: int = 10,
    ) -> str:
        """
        ...
        # Header
        markdown_parts.append(
            f"# TraceLens Comparison: {gpu1_name} (Baseline) vs {gpu2_name} (Target)\n"
        )
        markdown_parts.append("=" * 80)
        markdown_parts.append("\n\n")
        ...
        markdown_parts.append(
            f"- **Excluded sheets:** {', '.join(self.excluded_sheets)}\n"
        )
        markdown_parts.append("\n")
        markdown_parts.append("=" * 80)
        markdown_parts.append("\n\n")
        ...
        for sheet_name in all_sheets:
            ...
            if has_gpu1 and has_gpu2:
                markdown_parts.append(
                    f"**Status:** Available in both {gpu1_name} and {gpu2_name}\n\n"
                )
            elif has_gpu1:
            ...
            if has_gpu1:
                ...
                row_limit = (
                    max_rows_ops_unique_args
                    if sheet_name == "ops_unique_args"
                    else max_rows_per_sheet
                )
                df1_display = df1.head(row_limit) if len(df1) > row_limit else df1

                markdown_parts.append(
                    f"**Rows:** {len(df1)} | **Columns:** {len(df1.columns)}"
                )
                ...
                try:
                    ...
                    # Format numerics
                    for col in df1_formatted.select_dtypes(
                        include=["float64", "float32"]
                    ).columns:
                        df1_formatted[col] = df1_formatted[col].apply(
                            lambda x: f"{x:.4f}" if pd.notna(x) else ""
                    ...
                    if sheet_name == "ops_unique_args":
                        cols_to_drop = [
                            col
                            for col in df1_formatted.columns
                            if df1_formatted[col]
                            .apply(lambda x: isinstance(x, (list, dict)))
                            .any()
                        ]
                        if cols_to_drop:
            ...
            if has_gpu2:
                ...
                markdown_parts.append(
                    f"**Rows:** {len(df2)} | **Columns:** {len(df2.columns)}"
                )
                ...
                try:
                    ...
                    # Format numerics
                    for col in df2_formatted.select_dtypes(
                        include=["float64", "float32"]
                    ).columns:
                        df2_formatted[col] = df2_formatted[col].apply(
                            lambda x: f"{x:.4f}" if pd.notna(x) else ""
                    ...
                    if sheet_name == "ops_unique_args":
                        cols_to_drop = [
                            col
                            for col in df2_formatted.columns
                            if df2_formatted[col]
                            .apply(lambda x: isinstance(x, (list, dict)))
                            .any()
                        ]
                        if cols_to_drop:
        ...
        if (
            "ops_summary_by_category" in gpu1_sheets
            and "ops_summary_by_category" in gpu2_sheets
        ):
            ...
            markdown_parts.append(
                f"Gap Formula: Gap = {gpu1_name} (Baseline) - {gpu2_name} (Target)\n"
            )
            markdown_parts.append(
                "- **POSITIVE gap** = Baseline took MORE time (SLOWER)\n"
            )
            markdown_parts.append(
                "- **NEGATIVE gap** = Baseline took LESS time (FASTER)\n\n"
            )
            ...
            try:
                ...
                if cat_col and time_col:
                    ...
                    for _, row1 in df1.iterrows():
                        ...
                        baseline_pct = (
                            float(row1[pct_col])
                            if pct_col and pd.notna(row1.get(pct_col))
                            else None
                        )
                        ...
                        if not row2_match.empty:
                            ...
                            target_pct = (
                                float(row2_match.iloc[0][pct_col])
                                if pct_col and pd.notna(row2_match.iloc[0].get(pct_col))
                                else None
                            )
                    ...
                    comparison_data.sort(
                        key=lambda x: abs(float(x["Gap (ms)"])), reverse=True
                    )
                    ...
                    markdown_parts.append("\n\n")
                    markdown_parts.append(
                        "**IMPORTANT:** Use these pre-calculated Gap values directly in your analysis.\n"
                    )
                    markdown_parts.append(
                        "Do NOT recalculate gaps from the individual tables above.\n\n"
                    )
                else:
                    markdown_parts.append(
                        "*Could not generate category comparison: required columns not found*\n\n"
                    )

            except Exception as e:
                markdown_parts.append(
                    f"*Error generating category comparison: {e}*\n\n"
                )
        ...
        markdown_parts.append("---\n")
        markdown_parts.append(
            f"*Comparison: {gpu1_name} (Baseline) vs {gpu2_name} (Target) | Excluded: {', '.join(self.excluded_sheets)}*\n"
        )
...
def generate_tracelens_markdown(
    result_dfs: Dict[str, pd.DataFrame],
    output_path: Path,
    gpu_name: str = "GPU",
    excluded_sheets: Optional[List[str]] = None,
