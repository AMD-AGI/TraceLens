...
def test_resolve_source_files_uses_cached_hub_snapshot(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("TraceLens.ModelUtils.source._hub_cache_root", lambda: tmp_path.parent)
    monkeypatch.setattr(
...
def _stub_upstream_transformers(
    ...
    monkeypatch.setattr("TraceLens.ModelUtils.source.fetch_github_source", fake_fetch)
    monkeypatch.setattr("TraceLens.ModelUtils.source._hub_snapshot_root", lambda model_id: None)
    monkeypatch.setattr(
