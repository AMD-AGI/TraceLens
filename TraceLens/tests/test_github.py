...
def test_find_modeling_files_walks_nested_python(tmp_path: Path):
    from TraceLens.ModelUtils.github import find_modeling_files
...
def test_resolve_source_files_uses_cached_hub_snapshot(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "TraceLens.ModelUtils.source._hub_cache_root", lambda: tmp_path.parent
    )
    monkeypatch.setattr(
...
def _stub_upstream_transformers(
    ...
    # of whether the installed transformers package has since started bundling the
    # model natively.
    monkeypatch.setattr(
        "TraceLens.ModelUtils.source._transformers_modeling_path",
