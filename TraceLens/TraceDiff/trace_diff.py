...
class TraceDiff:
    ...
    @staticmethod
    @functools.lru_cache(maxsize=4096) # eliminating regex work for repeated names (vast majority of calls)
    def _normalize_name_for_comparison(name):
        ...
        normalized = _RE_HEX_ADDR.sub("0xXXXX", name) # replace with pre-compiled
        # Remove line numbers from Python stack frames (filename.py(line_number): function)
        #normalized = re.sub(r"\.py\(\d+\):", ".py:", normalized)
        normalized = _RE_PY_LINENO.sub(".py:", normalized) # replace with pre-compiled
        return normalized
