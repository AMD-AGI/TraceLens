...
def _check_node(node: dict[str, Any]) -> list[str]:
    ...
    if input_types is None:
        return []
    tensor_count = sum(1 for t in input_types if t != "Scalar")
    node_id = node.get("id")
    warnings: list[str] = []

    if op in _SINGLE_TENSOR_AXIS_OPS and tensor_count > 1:
        # These ops take a single tensor plus a scalar ``dim``; a second tensor
        # operand (a graph edge) means a caller argument was mis-wired onto the op
        # (e.g. ``position_ids[..., None]`` also receiving ``hidden_states``). A
        # count of 0 is left alone: the sole operand may be a legitimately hidden
        # constant/buffer, which the "never show constants" rule prunes.
        warnings.append(
            f"{node_id} [{op}]: takes 1 tensor operand + a scalar dim, but got "
            f"{tensor_count} tensor operands (input_types={input_types}). A scalar "
            f"dim argument must not be wired as a tensor edge."
        )
    ...
    if op in _SAME_RANK_OPS and input_shapes is not None:
        ranks = {
            ...
            if typ != "Scalar" and isinstance(shape, list)
        }
        if len(ranks) > 1:
            warnings.append(
