...
def is_rocm_gemm(kernel_name):
    """
    Check if a kernel name matches the stricter ROCm GEMM naming pattern:
    e.g., 'Cijk_Alik_Bljk_...' where each of C/A/B is followed by exactly
    three axis letters.
    """
...
def is_cuda_gemm(kernel_name):
    ...
    return kernel_name.starteswith('nvjet')
