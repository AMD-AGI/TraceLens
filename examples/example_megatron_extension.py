...
def tree_postprocess_extension(trace_tree):
    ...
    if (
        ...
        fwd_op_events = [
            ...
            for uid in trace_tree.name2event_uids["_Linear"]
        ]
        for fwd_op_event in fwd_op_events:
            create_host_mm_ops_from_linear_op(trace_tree, fwd_op_event)
    if (
        "_LayerNormLinear" in trace_tree.name2event_uids
        and "tex_ts::te_gemm_ts" not in trace_tree.name2event_uids
    ):
        fwd_op_events = [
            trace_tree.get_UID2event(uid)
            for uid in trace_tree.name2event_uids["_LayerNormLinear"]
        ]
        for fwd_op_event in fwd_op_events:
...
def _create_host_mm_ops_common(
    ...
    if not fwd_gpu_event_ids:
        logging.warning(
            f"[Warning] No GPU events found for fwd UID {fwd_op_event['UID']}"
        )
        return
    ...
    if len(fwd_gemm_kernels) != 1:
        logging.warning(
            f"[Warning] Expected 1 GEMM kernel in fwd, found {len(fwd_gemm_kernels)}"
        )
        return
    ...
    if not bwd_ops:
        logging.warning(
            f"[Warning] No backward op found for fwd UID {fwd_op_event['UID']}"
        )
        return
    ...
    try:
        ...
        logging.warning(
            f"[Warning] Missing shape info in fwd UID {fwd_op_event['UID']}: {e}"
        )
        return
    ...
    inject_pseudo_op(
        trace_tree,
        xgrad_kernel,
        f"{prefix}Backward_xgrad_mm",
        seq_num,
        [Y_grad_shape, W_shape],
        [inp_dtype, inp_dtype],
        [],
        [],
    )
    inject_pseudo_op(
        trace_tree,
        wgrad_kernel,
        f"{prefix}Backward_wgrad_mm",
        seq_num,
        [Y_grad_shape[::-1], X_shape],
        [inp_dtype, inp_dtype],
        [],
        [],
    )
...
def inject_pseudo_op(
    ...
    pseudo_evt = {
        ...
        "args": {
            ...
            "Input type": (
                orig_cpu_evt["args"]["Input type"] if types is None else types
            ),
            "Input Strides": (
                orig_cpu_evt["args"]["Input Strides"] if strides is None else strides
            ),
            "Concrete Inputs": (
                orig_cpu_evt["args"]["Concrete Inputs"]
...
class tev2_pseudo_gemm(GEMM):
    ...
    def get_param_details(event):
        ...
        if not isinstance(input_type, (list, tuple)) or len(input_type) < 2:
            raise ValueError(
                f"Expected 'Input type' in event['args'] to be a list or tuple with at least 2 elements, got: {input_type}"
            )
...
from TraceLens.PerfModel import extract_sdpa_cfg, GroupedGemm, SDPA
...
class custom_grouped_gemm(GroupedGemm):
    ...
    def get_param_details(event):
        ...
        if transpose:
            assert (
                K == W_dims[-1]
            ), f"Expected K dimension to match W_dims[-1] for transposed GEMM, got {K} and {W_dims[-1]}"
            G, N, K = W_dims
        else:
            assert (
                K == W_dims[1]
            ), f"Expected K dimension to match W_dims[1] for non-transposed GEMM, got {K} and {W_dims[1]}"
            G, K, N = W_dims
        ...
        assert (
            dtype_X == dtype_W
        ), f"Expected input types to match, got {dtype_X} and {dtype_W}"
