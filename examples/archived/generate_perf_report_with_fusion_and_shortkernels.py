...
def get_next_host_op(perf_analyzer, host_op):
    ...
    next_gpu_event = perf_analyzer.tree.dict_stream_index2event.get(
        (gpu_stream, next_index), None
    )
    ...
    assert (
        next_gpu_event_launcher is not None
    ), "No launcher event found for the next gpu event"
    next_gpu_event_host_op = perf_analyzer.tree.get_parent_event(
        next_gpu_event_launcher
    )
...
def get_df_fusion_opportunity(perf_analyzer, fusion_op_names):
    ...
    """
    fusion_candidates = [
        e for e in perf_analyzer.tree.events if e["name"] in fusion_op_names
    ]
    ...
    for evt in fusion_candidates:
        ...
        try:
            ...
            continue
        next_gpu_events = [
            perf_analyzer.tree.get_UID2event(uid)
            for uid in next_host_op.get("gpu_events", [])
        ]
        ...
        if last_kernel and first_next_kernel:
            dict_info = {
                ...
                "last kernel by fusion candidate stream": last_kernel["args"].get(
                    "stream"
                ),
                "last kernel by fusion candidate stream index": last_kernel["args"].get(
                    "stream_index"
                ),
                ...
                "first kernel by fusion post op stream": first_next_kernel["args"].get(
                    "stream"
                ),
                "first kernel by fusion post op stream index": first_next_kernel[
                    "args"
                ].get("stream_index"),
    ...
    if df_fusion_study.empty:
        ...
        return pd.DataFrame()
    df_fusion_study["combo"] = (
        df_fusion_study["fusion candidate"] + " -> " + df_fusion_study["fusion post op"]
    )
    df_fusion_study_grouped = (
        df_fusion_study.groupby(["combo"])
        .agg(
            {
                "first kernel by fusion post op duration": [
                    "mean",
                    "std",
                    "sum",
                    "count",
    ...
    df_fusion_study_grouped.columns = [
        "_".join(col).strip() for col in df_fusion_study_grouped.columns.values
    ]

    df_fusion_study_grouped["estimated saved time percent"] = (
        df_fusion_study_grouped["first kernel by fusion post op duration_sum"]
...
def main():
    ...
    parser.add_argument(
        "--fusion_op_names",
        type=str,
        default="aten::miopen_convolution,aten::miopen_batch_norm",
    ...
    if args.topk_ops is not None:
        df_kernel_launchers_unique_args = df_kernel_launchers_unique_args.head(
            args.topk_ops
        )
    ...
    for op_cat, op_names in dict_cat2names.items():
        ...
        if op_cat in ["GEMM", "UnaryElementwise", "BinaryElementwise"]:
            ...
            df_ops_bwd = perf_analyzer.build_df_perf_metrics(
                op_events, bwd=True, include_kernel_names=True, include_args=True
            )
    ...
    with pd.ExcelWriter(args.output_xlsx_path) as writer:
        ...
        df_kernel_launchers_summary.to_excel(
            writer, sheet_name="ops_summary", index=False
        )
        if args.topk_ops is not None:
        ...
        df_fusion_details.to_excel(
            writer, sheet_name="fusion_candidates_details", index=False
        )
