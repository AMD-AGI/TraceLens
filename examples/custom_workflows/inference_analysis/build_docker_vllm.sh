...
usage() {
    ...
    echo ""
    echo "  vllm-version    One of: v14, v15, v16  (shorthand for v0.14.0, v0.15.0, v0.16.0)"
    echo ""
    echo "Examples:"
...
case "${VLLM_VERSION}" in
    ...
    *)
        ...
        echo "Supported versions: v14, v15, v16"
        exit 1
        ;;
esac
...
RUN VLLM_DIR=\$(python -c "import vllm, os; print(os.path.join(os.path.dirname(vllm.__file__), '..'))") && \\
    cd "\${VLLM_DIR}" && \\
    git apply /tmp/TraceLens-internal/${PATCH_PATH} && \\
