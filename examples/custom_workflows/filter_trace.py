...
def get_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--annotation",
        type=str,
        help="Name of the user annotation to filter by",
        required=True,
    )
    parser.add_argument(
        "--output",
        type=str,
        help="Output file name with .json or .gz extension",
...
def find_user_annotation(
    event_iter: Iterable[dict[str, Any]], user_annotation_name: str
    ...
    for event in event_iter:
        ...
        if event.get("name", None) == user_annotation_name:
            logger.debug(
                f"Found user annotation event with name matching {user_annotation_name=}: {event}"
            )
            return event
    return None


def find_user_annotation_in_trace_file(
    json_file: Path, user_annotation_name: str
) -> Optional[dict[str, Any]]:
    with (
        gzip.open(json_file, "rt")
        if json_file.suffix == ".gz"
        else open(json_file, "r", encoding="UTF-8")
    ) as f:
        return find_user_annotation(
            ijson.items(f, "traceEvents.item"),
            user_annotation_name=user_annotation_name,
        )


def filter_trace_events_to_range(
    events: Iterable[dict[str, Any]], start: Decimal, end: Decimal
) -> list[dict[str, Any]]:
...
def main(
    ...
    if output_file.exists() and not allow_overwrite:
        logger.warning(
            f"Output file {output_file.name} already exists. Use --allow-overwrite to overwrite."
        )
        return
    ...
    with (
        gzip.open(trace_file, "rt")
        if trace_file.suffix == ".gz"
        else open(trace_file, "rb")
    ) as f:
        data = json.load(f)
    ...
    user_annotation_event = find_user_annotation(
        data["traceEvents"], user_annotation_name=user_annotation_name
    )
    if user_annotation_event is None:
        logger.warning(
            f"Could not find event with 'cat': 'user_annotation' and 'name': {user_annotation_name}"
        )
        return
    ...
    with (
        ...
        else open(output_file, "wt", encoding="UTF-8")
    ) as fout:
        json.dump(data, fout)
...
if __name__ == "__main__":
    ...
    main(
        trace_file=args.filename,
        user_annotation_name=args.annotation,
