...
from TraceLens.PerfModel import dict_cat2names, SDPA
...
class transformer_engine_attention(SDPA):
    ...
    def get_param_details(event):
        ...
        assert (
            k_shape == v_shape
        ), f"Key and value shapes are different: {k_shape} != {v_shape}"
...
def main():
    ...
    perf_analyzer = TreePerfAnalyzer.from_file(
        profile_filepath=args.profile_json_path, arch=gpu_arch_json
    )
    ...
    with pd.ExcelWriter(args.output_xlsx_path) as writer:
        ...
        df_kernel_launchers_unique_args.to_excel(
            writer, sheet_name="kernel_launchers_unique_args", index=False
        )
