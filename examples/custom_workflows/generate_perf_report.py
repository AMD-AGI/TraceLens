...
def process_single_trace(args):
    ...
    kernel_events = [
        ...
        if perf_analyzer.event_to_category(e) in kernel_categories
    ]
    kernel_events_linked = [
        e for e in kernel_events if e.get("args", {}).get("External id") is not None
    ]
    kernel_events_unlinked = [
        e for e in kernel_events if e.get("args", {}).get("External id") is None
    ]

    short_kernel_events_linked = [
        e for e in kernel_events_linked if e.get("dur", 0) < 10
    ]
    short_kernel_events_unlinked = [
        e for e in kernel_events_unlinked if e.get("dur", 0) < 10
    ]

    # OBS! 'kernel' category only
    df_kernel_events = perf_analyzer.get_df_kernels()
    df_short_kernel_events = df_kernel_events[
        df_kernel_events["Kernel duration (µs)"] < 10
    ]
    short_cpu_op_counts = (
        df_short_kernel_events["Parent cpu_op"].value_counts().to_dict()
    )
    ...
    cuda_events = [
        e
        for e in perf_analyzer.tree.events
        if perf_analyzer.event_to_category(e) in cuda_categories
    ]
...
def analyze_traces(
    ...
    for parent_dirpath, filenames in all_traces_grouped_sorted.items():
        ...
        xlsx_path = xlsx_path or osp.join(
            parent_dirpath, f"{prefix}_performance_report.xlsx"
        )
        ...
        for result in results:
            ...
            df_kernel_launchers_all = (
                pd.concat([df_kernel_launchers_all, result["df_kernel_launchers"]])
                if df_kernel_launchers_all is not None
                else result["df_kernel_launchers"]
            )
            ...
            df_gpu_timelines_all = (
                pd.concat([df_gpu_timelines_all, df_gpu_timeline.iloc[:, 1:]], axis=1)
                if df_gpu_timelines_all is not None
                else df_gpu_timeline
            )
            ...
            linked_short_kernels_all.append(
                {
                    ...
                    "num_short_kernel_events_linked": result[
                        "num_short_kernel_events_linked"
                    ],
                    "num_short_kernel_events_unlinked": result[
                        "num_short_kernel_events_unlinked"
                    ],
                }
            )
        ...
        df_gpu_timelines_all["time_ms_avg"] = df_gpu_timelines_all.loc[
            :, df_gpu_timelines_all.columns.str.contains("time_ms")
        ].mean(axis=1)
        df_gpu_timelines_all["time_ms_std"] = df_gpu_timelines_all.loc[
            :, df_gpu_timelines_all.columns.str.contains("time_ms")
        ].std(axis=1)
        ...
        for group, df_all in dfs_all.items():
            ...
            df_all_summary = summarize_df_perf_metrics(
                df_all, agg_metrics=["mean", "std"]
            )
            df_all_summary["kernel_time_sum_cum_pct"] = (
                df_all_summary["Kernel Time (µs)_sum"].cumsum()
                / df_all_summary["Kernel Time (µs)_sum"].sum()
            )
            ...
            if node_replay and group in ["gemm", "conv"]:
                df_node_replay_results = run_node_replay(
                    group, df_all_summary, parent_dirpath
                )

                with pd.ExcelWriter(xlsx_path, mode=get_xlsx_mode(xlsx_path)) as writer:
                    df_node_replay_results.to_excel(
        ...
        with pd.ExcelWriter(xlsx_path, mode=get_xlsx_mode(xlsx_path)) as writer:
            df_kernel_launchers_all_summary = build_kernel_launchers_summary(
                df_kernel_launchers_all, world_size
            )

            df_kernel_launchers_all_summary.to_excel(
                writer, sheet_name="kernel_launchers", index=False
            )
            df_gpu_timelines_all.to_excel(
                writer, sheet_name="gpu_timelines", index=False
            )

        # Build and write (un)linked short kernel info
        df_linked_short_kernels_all = pd.DataFrame(linked_short_kernels_all)
        with pd.ExcelWriter(xlsx_path, mode=get_xlsx_mode(xlsx_path)) as writer:
            df_linked_short_kernels_all.to_excel(
                writer, sheet_name="linked_and_short_kernels", index=False
            )

        if unlinked_kernel_events_all:
            df_unlinked_kernel_events_all = pd.DataFrame(unlinked_kernel_events_all)
            df_unlinked_summary = (
                df_unlinked_kernel_events_all.groupby(["name", "cat"])
                .agg(
                    count=("dur", "size"),
                    min_dur=("dur", "min"),
                    max_dur=("dur", "max"),
            ...
            with pd.ExcelWriter(xlsx_path, mode=get_xlsx_mode(xlsx_path)) as writer:
                df_unlinked_summary.to_excel(
                    writer, sheet_name="unlinked_short_kernels_summary", index=False
                )
        else:
            print("No unlinked kernel events found.")

        if short_cpu_op_counts_all:
            df_short_cpu_op_counts_all = pd.DataFrame.from_dict(
                short_cpu_op_counts_all, orient="index", columns=["count"]
            )
            df_short_cpu_op_counts_all.sort_values(
            ...
            with pd.ExcelWriter(xlsx_path, mode=get_xlsx_mode(xlsx_path)) as writer:
                df_short_cpu_op_counts_all.to_excel(
                    writer, sheet_name="short_cpu_op_counts", index=True
                )
        else:
        ...
        if cuda_events_all:
            ...
            df_cuda_summary_by_rank = (
                df_cuda_events_all.groupby(["name", "cat", "rank"])
                .agg(
                    count=("dur", "size"),
                    min_dur=("dur", "min"),
                    max_dur=("dur", "max"),
            ...
            with pd.ExcelWriter(xlsx_path, mode=get_xlsx_mode(xlsx_path)) as writer:
                df_cuda_summary.to_excel(
                    writer, sheet_name="cuda_events_summary", index=True
                )

        # Build and write high-level grouped breakdown
        df_grouped_breakdown = build_grouped_breakdown(
            df_kernel_launchers_all_summary, df_gpu_timelines_all
        )

        with pd.ExcelWriter(xlsx_path, mode=get_xlsx_mode(xlsx_path)) as writer:
            df_grouped_breakdown.to_excel(
                writer, sheet_name="grouped_breakdown", index=False
            )
        ...
        if world_size > 1:
            ...
            start_time = time.perf_counter()
            all_filepaths = [
                osp.join(parent_dirpath, filename) for filename in filenames
            ]
            ...
            with pd.ExcelWriter(xlsx_path, mode="a") as writer:
                nccl_analyser.build_df_summary_nccl_implicit_sync_cat(
                    agg_metrics=["mean"], strict_metadata_check=False
                ).to_excel(
                    writer, sheet_name="summary_nccl_implicit_sync_cat", index=False
                )
                nccl_analyser.build_df_long().to_excel(
                    writer, sheet_name="long", index=False
                )
                nccl_analyser.build_df_nccl_implicit_sync_cat(
                    strict_metadata_check=False
                ).to_excel(writer, sheet_name="nccl_implicit_sync_cat", index=False)
                nccl_analyser.build_df_nccl_implicit_sync_cat(
                    detailed=True, strict_metadata_check=False
                ).to_excel(
                    writer, sheet_name="nccl_implicit_sync_cat_detailed", index=False
                )
                df_all2allv = nccl_analyser.build_df_nccl_all2allv(
                    strict_metadata_check=False
                )
                if df_all2allv is not None:
                    df_all2allv.to_excel(
                        writer, sheet_name="nccl_all2allv", index=False
                    )
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=["rank_0"],
        help="Select files containing given substring(s) in their full filepaths.",
    )
    parser.add_argument(
        "-r",
        action="store_true",
        help="Run node replay for GEMMs and CONVs that contribute 99 pct to group-specific execution time.",
    )
    parser.add_argument(
        "-d",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Filepath to save the Excel performance report. Note that this works only with a single base/parent directory containing one set of traces.",
    )
