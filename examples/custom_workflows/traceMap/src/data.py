...
class TraceDataProcessor:
    ...
    def create_top_n_data(df, n=30):
        ...
        kernel_stats = (
            df.groupby("Kernel Name")
            .agg({"Duration (us)": ["sum", "count", "mean"]})
            .round(3)
        )
        kernel_stats.columns = ["Total Duration (us)", "Count", "Avg Duration (us)"]
        kernel_stats = kernel_stats.sort_values(
            "Total Duration (us)", ascending=False
        ).head(n)
    ...
    def create_sorted_latency_data(df):
        ...
        sorted_df = df.sort_values("Duration (us)", ascending=False).reset_index(
            drop=True
        )
        return sorted_df
...
class DataSourceManager:
    ...
    def _create_all_sources(self):
        ...
        self.source_gpu_a_filtered = ColumnDataSource(
            self.df_gpu_a.head(self.default_window_size)
        )
        self.source_gpu_b_filtered = ColumnDataSource(
            self.df_gpu_b.head(self.default_window_size)
        )

        # Sorted filtered sources
        initial_gpu_a_sorted = (
            self.df_gpu_a.head(self.default_window_size)
            .sort_values("Duration (us)", ascending=False)
            .reset_index(drop=True)
        )
        initial_gpu_b_sorted = (
            self.df_gpu_b.head(self.default_window_size)
            .sort_values("Duration (us)", ascending=False)
            .reset_index(drop=True)
        )
        ...
        self.source_gpu_a_combined_filtered = ColumnDataSource(
            self.df_gpu_a.head(self.default_window_size)
        )
        self.source_gpu_b_combined_filtered = ColumnDataSource(
            self.df_gpu_b.head(self.default_window_size)
        )

        self.source_sorted_gpu_a_combined_filtered = ColumnDataSource(
            initial_gpu_a_sorted
        )
        self.source_sorted_gpu_b_combined_filtered = ColumnDataSource(
            initial_gpu_b_sorted
        )
    ...
    def _create_top_n_sources(self):
        ...
        top_n_both = TraceDataProcessor.create_top_n_data(
            pd.concat([self.df_gpu_a, self.df_gpu_b], ignore_index=True)
        )
