...
from src.data import DataSourceManager, TraceDataProcessor
...
class TableBuilder:
    ...
    def create_top_n_table_columns():
        ...
        return [
            ...
            TableColumn(
                field="Total Duration (us)",
                title="Total Duration (μs)",
                width=120,
                formatter=NumberFormatter(format="0,0.000"),
            ),
            TableColumn(field="Count", title="Count", width=60),
            TableColumn(
                field="Avg Duration (us)",
    ...
    def create_kernel_table(source, width=2000):
        ...
        return DataTable(
            source=source, columns=columns, width=width, height=600, index_position=None
        )
...
class GPUTraceDashboard:
    ...
    def __init__(
        self, trace_path1, trace_path2, gpu_name_a="GPU_A", gpu_name_b="GPU_B"
    ):
        ...
        self.data_sources = DataSourceManager(
            self.df_gpu_a, self.df_gpu_b, self.default_window_size
        )
    ...
    def _create_charts(self):
        ...
        # Combined charts
        self.chart_gpu_a_combined, self.bars_gpu_a_combined = (
            ChartBuilder.create_bar_chart(
                f"{self.gpu_name_a} Kernel Latency",
        ...
        self.chart_gpu_b_combined, self.bars_gpu_b_combined = (
            ChartBuilder.create_bar_chart(
                f"{self.gpu_name_b} Kernel Latency",
    ...
    def _create_tables(self):
        ...
        # Individual tables
        self.table_gpu_a = TableBuilder.create_kernel_table(
            self.data_sources.source_gpu_a_filtered
        )
        self.table_gpu_b = TableBuilder.create_kernel_table(
            self.data_sources.source_gpu_b_filtered
        )

        # Sorted tables
        self.sorted_table_gpu_a = TableBuilder.create_kernel_table(
            self.data_sources.source_sorted_gpu_a_filtered
        )
        self.sorted_table_gpu_b = TableBuilder.create_kernel_table(
            self.data_sources.source_sorted_gpu_b_filtered
        )

        # Combined tables
        self.table_gpu_a_combined = TableBuilder.create_kernel_table(
            self.data_sources.source_gpu_a_combined_filtered, width=1000
        )
        self.table_gpu_b_combined = TableBuilder.create_kernel_table(
            self.data_sources.source_gpu_b_combined_filtered, width=1000
        )

        # Combined sorted tables
        self.sorted_table_gpu_a_combined = TableBuilder.create_kernel_table(
            self.data_sources.source_sorted_gpu_a_combined_filtered, width=1000
        )
        self.sorted_table_gpu_b_combined = TableBuilder.create_kernel_table(
            self.data_sources.source_sorted_gpu_b_combined_filtered, width=1000
        )

        # Top N tables
        self.top_table_gpu_a = TableBuilder.create_top_n_table(
            self.data_sources.source_top_gpu_a
        )
        self.top_table_gpu_b = TableBuilder.create_top_n_table(
            self.data_sources.source_top_gpu_b
        )
        self.top_table_both = TableBuilder.create_top_n_table(
            self.data_sources.source_top_both, width=800
        )

        # Combined top N tables
        self.top_table_gpu_a_combined = TableBuilder.create_top_n_table(
            self.data_sources.source_top_gpu_a, width=1000
        )
        self.top_table_gpu_b_combined = TableBuilder.create_top_n_table(
            self.data_sources.source_top_gpu_b, width=1000
        )

    def _create_controls(self):
        """Create all controls for the dashboard."""
        # Window size controls
        self.window_size_spinner = ControlsBuilder.create_window_size_spinner(
            self.default_window_size
        )
        self.window_size_spinner_combined = ControlsBuilder.create_window_size_spinner(
            self.default_window_size
        )

        # Sliders
        self.slider_gpu_a = ControlsBuilder.create_slider(
            len(self.df_gpu_a), self.default_window_size, self.gpu_name_a
        )
        self.slider_gpu_b = ControlsBuilder.create_slider(
            len(self.df_gpu_b), self.default_window_size, self.gpu_name_b
        )
        self.slider_gpu_a_combined = ControlsBuilder.create_slider(
            len(self.df_gpu_a), self.default_window_size, self.gpu_name_a
        )
        self.slider_gpu_b_combined = ControlsBuilder.create_slider(
            len(self.df_gpu_b), self.default_window_size, self.gpu_name_b
        )
    ...
    def _attach_callbacks(self):
        ...
        self.window_size_spinner_combined.js_on_change(
            "value", window_size_callback_combined
        )
        ...
        slider_callback_gpu_a = CallbackManager.create_slider_callback(
            ...
            {"slider": self.slider_gpu_a, "spinner": self.window_size_spinner},
        )
        ...
        slider_callback_gpu_b = CallbackManager.create_slider_callback(
            ...
            {"slider": self.slider_gpu_b, "spinner": self.window_size_spinner},
        )
        ...
        self.bars_gpu_a_combined.data_source.selected.js_on_change(
            "indices", tap_callback_gpu_a_combined
        )
        self.bars_gpu_b_combined.data_source.selected.js_on_change(
            "indices", tap_callback_gpu_b_combined
        )
