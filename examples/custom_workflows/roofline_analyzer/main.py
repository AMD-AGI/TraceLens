...
from cli import display_summary, read_toml_config
from roofline import calculate_ridge_points, calculate_rooflines, plot_roofline
from xlsx import add_analysis_columns, export_to_xlsx, read_xlsx
...
def analyze(
    excel_file: Path = typer.Argument(
        ..., help="Path to the Excel file containing kernel data"
    ),
    config_file: Path = typer.Argument(..., help="Path to the TOML config file"),
    plot_output: Optional[Path] = typer.Option(
        None, help="Path to save a separate copy of the roofline plot"
    ),
    ...
    try:
        ...
        output_file = export_to_xlsx(
            df_result, df_orig, config, ai_ridge_mtf, ai_ridge_maf, plot_fig
        )

        # Display summary
        display_summary(
            df_orig, df_result, output_file, config, ai_ridge_mtf, ai_ridge_maf
        )
