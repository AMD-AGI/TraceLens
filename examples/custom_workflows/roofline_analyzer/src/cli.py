    ...
    try:
        ...
        required_sections = {
            "accelerator": [
                ...
                "max_compute_teraflops",
                "max_achievable_teraflops",
            ],
        ...
        for section, keys in required_sections.items():
            ...
            for key in keys:
                if key not in config[section]:
                    raise ValueError(
                        f"Missing required key in config[{section}]: {key}"
                    )
...
def display_summary(
    ...
    hw_table.add_row(
        "Max Memory Bandwidth", f"{config['accelerator']['max_memory_bandwidth']} TB/s"
    )
    hw_table.add_row(
        "Max Compute", f"{config['accelerator']['max_compute_teraflops']} TFLOPS"
    )
    hw_table.add_row(
        "Max Achievable Compute",
        f"{config['accelerator']['max_achievable_teraflops']} TFLOPS",
    )
