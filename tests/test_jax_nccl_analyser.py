...
class TestJaxNcclAnalyserLoadTraceData(unittest.TestCase):
    ...
    def test_load_trace_data_empty_node_mapping(self):
        ...
        with patch.object(JaxNcclAnalyser, 'load_trace_data'), \
             patch.object(JaxNcclAnalyser, 'build_collectives_df_through_xla'):
...
class TestJaxNcclAnalyserGetBusBandwidthScaler(unittest.TestCase):
    ...
    def test_get_bus_bandwidth_scaler_basic(self):
        ...
        self.assertAlmostEqual(
            self.analyser.get_bus_bandwidth_scaler('all-reduce', 4), 
            1.5, places=2
        )
        
        # Test all-gather (should return 0.75)  
        self.assertAlmostEqual(
            self.analyser.get_bus_bandwidth_scaler('all-gather', 4), 
            0.75, places=2
        )
        
        # Test all-to-all (should return 0.75)
        self.assertAlmostEqual(
            self.analyser.get_bus_bandwidth_scaler('all-to-all', 4), 
            0.75, places=2
        )
        
        # Test reduce-scatter (should return 0.75)
        self.assertAlmostEqual(
            self.analyser.get_bus_bandwidth_scaler('reduce-scatter', 4), 
            0.75, places=2
        )
        
        # Test collective-permute (should return 1.0)
        self.assertEqual(
            self.analyser.get_bus_bandwidth_scaler('collective-permute', 4), 
            1.0
        )
        
        # Test unknown collective (should return default 1.0)
        self.assertEqual(
            self.analyser.get_bus_bandwidth_scaler('unknown-op', 4), 
            1.0
        )
...
class TestJaxNcclAnalyserAnalyzeAllCollectivesFromDf(unittest.TestCase):
    ...
    def test_analyze_all_collectives_from_df_basic(self, mock_print):
        ...
        self.assertAlmostEqual(group_0['algorithmic_bandwidth_gbps'], 49.681, delta = 0.001)
