...
RESNET = os.path.join(os.path.dirname(__file__), "traces/mi300/resnet_act_checkpoint.json.gz")
...
class TestTreePerfPhase13:
    ...
    def test_unified_table_with_perf_metrics(self):
        analyzer = TreePerfAnalyzer.from_file(
            RESNET, rebuild_tree=True, enable_pseudo_ops=True, detect_recompute=True,
        )
