...
from tests.test_perfmodel_coverage import _GDN_ANNOTATION, _moe_unfused_event
...
class TestTreePerfFromFileCapture:
    ...
    def test_from_file_with_capture_merge(self):
        ...
        merged = merge_capture_trace_into_graph(capture, metadata, os.path.join(case, trace_gz))
...
class TestCaptureMergeIntegration:
    def test_merge_synthetic_capture_graph(self, tmp_path):
        graph_events = {
            "traceEvents": [
                {"ph": "X", "name": "hipGraphLaunch", "cat": "cuda_runtime", "ts": 0, "dur": 100, "args": {}},
                {"ph": "X", "name": "gemm_k", "cat": "kernel", "ts": 50, "dur": 40,
                 "args": {"stream": 7}},
            ]
        }
        ...
        cap_dir.mkdir()
        (cap_dir / "execution_details.json").write_text(json.dumps([{
