...
from model_explorer_export.styles import OPERATION_COLORS, ROLE_COLORS, ensure_readable_text

_DEFAULT_OUTPUT_ID = "0"

_ROLE_COLORS = ROLE_COLORS
_OPERATION_COLORS = OPERATION_COLORS
...
def _node_style(spec) -> dict[str, str] | None:
    block = spec.block
    if block is not None and block.role in _ROLE_COLORS:
        return ensure_readable_text(dict(_ROLE_COLORS[block.role]))
    operation = classify_operation(block, synthetic=spec.synthetic, label=spec.label)
    if operation is not None and operation.value in _OPERATION_COLORS:
        return ensure_readable_text(dict(_OPERATION_COLORS[operation.value]))
    return None
