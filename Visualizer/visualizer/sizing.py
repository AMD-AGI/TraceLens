...
# Band above a frame's top border that its caption occupies.
INLINE_FRAME_CAPTION_BAND = INLINE_FRAME_LABEL_GAP + INLINE_FRAME_LABEL_LINE_H
...
def estimate_block_size(
    ...
    *,
    ...
    if label.strip() in {"×", "x", "*", "⨉"}:
        width = max(width, estimate_block_size("Multiply", pad_x=pad_x, pad_y=pad_y)[0])
